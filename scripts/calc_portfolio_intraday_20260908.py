# -*- coding: utf-8 -*-
"""2026-09-08 盘中: 组合当日收益估算（9/7 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 9/8 盘中涨跌（%）：场内ETF实时(13:45 akshare) + 指数实时(13:45 akshare / 13:43 hq 直连)
intraday = {
    # 场内ETF (13:45 实时)
    "513050": -0.38, "159928": -0.15, "512170": 0.89, "513180": -1.23,
    "159920": -0.60, "512880": -0.37, "512980": 0.12, "515180": 1.18,
    "159938": 0.47,
    # 指数 (13:45 akshare, hq 直连交叉验证)
    "sh000001": 0.31, "sz399006": -0.75, "sh000932": 0.07, "sh000300": -0.12,
    "HSTECH": -1.02, "HSI": -0.18,
    # 个股实时 (sina hq 13:43)
    "002410": 0.57,   # 广联达 8.86 (盘中最高8.94仍未破9.0)
    "600438": 0.94,   # 通威 11.78
    # 美股医药 QDII: XLV 9/4 -1.04% 已于 9/8 早盘兑现落地; 下批对应美东9/8重开于9/9-10兑现 → 盘中估算按 0
    "XLV": 0.0, "IYH": 0.0,
    # 光伏代理(清洁能源基金参考) 通威+0.94% → 光伏ETF代理
    "515790": 0.94,
}

# 持仓基准: 9/7 收盘口径市值 (portfolio_preopen_20260908.json)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260908.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 +0.68%
...
def approx_pct(code, name):
    ...
    if code6 in ("002708", "161616", "001180", "001551", "012323", "000727"):
        return MED_AVG                  # A股医药 -> 医疗/医药ETF均值 +0.68%
    if code6 in ("000248", "519915", "000968"):
        return intraday["sh000932"]     # 大消费 -> 中证消费 +0.07%
    if code6 in ("004424",):
        return intraday["512980"]       # 文体娱乐 -> 传媒ETF
    if code6 in ("012348",):
        return intraday["513180"]       # 恒生科技QDII -> 恒指科技ETF -1.23%
    if code6 in ("164906",):
        return intraday["513050"]       # 中概互联 -> 中概互联ETF -0.38%
    if code6 in ("000369", "016280"):
        return intraday["IYH"]          # 美股医药QDII -> IYH(今日兑现按0)
    if code6 in ("000071",):
        return intraday["159920"]       # 恒生ETF联接 -> 恒生ETF华夏 -0.60%
    if code6 in ("000051", "110020"):
        return intraday["sh000300"]     # 沪深300 -> 沪深300指数 -0.12%
    if code6 in ("001469", "001552"):
        return intraday["512880"]       # 金融地产/证券保险 -> 证券ETF -0.37%
    if code6 in ("005368",):
        return intraday["515790"]       # 清洁能源 -> 光伏代理(通威)
    if code6 in ("100032",):
        return intraday["515180"]       # 红利 -> 100红利 +1.18%
    if code6 in ("004752",):
...
print(f"总资产(9/7收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(9/7收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260908.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260908.json")

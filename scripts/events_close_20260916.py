...
"""2026-09-16 盘后档事件库处理：
  ① 追加盘后 23 条事件
  ② 回填 9/16 当日 83 条 actual_ret_1d（美股标普医药留空，待 9/17 盘前档兜底）
  ③ 全库完整性扫描
  ④ 1日样本 + 方向验证 + 3/5/10日窗口统计 → history/event_stats_20260916.json（收盘口径覆盖）
统计口径与 scripts/event_stats_20260916.py（盘前档）保持一致
"""
...
track_ret = {
    'A股医药': 0.69,     # ETF均值（医药ETF广发 +0.48% / 医疗ETF +0.90%）；申万医药生物 +0.52%
    '大消费': -0.83,     # 中证消费 12,151.82 -0.83%（收盘破 12,200 防线）
    '恒生科技': 0.79,    # HSTECH 4,325.45 +0.79%（收复 4,300）
    '宏观': 0.71,        # 上证指数 3,891.60 +0.71%
    '其他/宽基': 0.45,   # 个股均值（广联达 +0.36% / 通威 +0.54%）
    # 美股标普医药：9/16 美股未收盘 → 留空待 9/17 盘前档兜底（§3.70）
}
REF = {
    'A股医药': '9/16 收盘：医药ETF广发 +0.48% / 医疗ETF +0.90%（申万医药生物 +0.52%）',
    '大消费': '9/16 收盘：中证消费 12,151.82 -0.83%',
    '恒生科技': '9/16 收盘：HSTECH 4,325.45 +0.79%',
    '宏观': '9/16 收盘：上证指数 3,891.60 +0.71%',
    '其他/宽基': '9/16 收盘：个股均值（广联达 +0.36% / 通威 +0.54%）',
}

# ---------- ① 追加盘后事件 ----------
news = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-09-16.json'), encoding='utf-8'))
...
print(f'回填 {filled} 条；留空 {blank} 条（美股标普医药，待 9/17 盘前档按 XLV 9/16 收盘兜底）')
...
out = {'date': TODAY, 'as_of': '2026-09-16收盘回填', 'library_total': len(all_ev),

# -*- coding: utf-8 -*-
"""盘前档 2026-08-18(周二) 行情抓取：场外基金净值增量(8/17周一净值) + 隔夜美股(8/17收盘) + 恒科/A股指数参考 + 且慢pmdj复测(第11日)"""
...
# 美股指数（.IXIC/.DJI，间歇 IndexError 用 QQQ/DIA 代理）
...
# ---------- 4. 港股指数 spot（盘前重试，预期失败则用已归档 8/17 收盘） ----------
hk_done = {}
...
for attempt in range(3):
    ...
    time.sleep(2)
if len(hk_done) < 2:
    print('  WARN 港股 spot 盘前仍空，沿用 indices.csv 8/17 收盘')
...
with open(os.path.join(HIST, 'preopen_20260818.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> preopen_20260818.json')

# -*- coding: utf-8 -*-
"""持仓合并 + 赛道占比 + 盘中组合估算收益 (2026-08-13 13:45 档)"""
...
# sean 最新快照 8/5 (8/12 无新快照), jasy 8/12, stock 8/12
...
# ---------- 盘中代理涨跌幅 (13:45 实时, 来自 fetch 结果) ----------
# 实时: 医疗ETF +2.29 / 医药ETF +1.61; 消费ETF +0.44 / 中证消费 +0.49
# 恒科: HSTECH +1.27 / 恒指科技ETF +0.66 / 中概互联 -0.88 / 恒生ETF +0.20
# A股: 上证 +0.47 / 创业板 +1.54
PROXY = {
    "A股医药": 1.95,         # 医疗ETF +2.29 / 医药ETF +1.61 均值 (场外 T+1 仍显示 8/12)
    "大消费": 0.47,          # 消费ETF +0.44 / 中证消费 +0.49 均值
    "美股标普医药": 0.0,     # QDII T+1, 8/12 美股 IYH +0.39% 将在明后日净值体现
    "恒生科技": 0.35,        # HSTECH +1.27 / 恒指科技 +0.66 / 中概互联 -0.88 / 恒生ETF +0.20 均值
    "其他/宽基": 0.47,       # 上证 +0.47 (宽基近似)
    "现金": 0.0,
}
ETF_REALTIME = {
    "159938": 1.61, "512170": 2.29, "159928": 0.44, "513180": 0.66,
    "513050": -0.88, "159920": 0.20, "512880": 1.00, "512980": -0.46,
    "515180": -1.19,
}
# 个股盘中价暂缺, 用指数近似: 广联达(软件)→创业板指 +1.54, 通威(光伏)→上证 +0.47 保守
STOCK_PROXY = {"002410": 1.54, "600438": 0.47}
def est_pct(row):
...
with open(os.path.join(BASE, "data/processed/history/portfolio_20260813.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),

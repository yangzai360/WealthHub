# -*- coding: utf-8 -*-
"""盘后档 2026-08-24：DeepSeek 情绪标注盘后新增 8 条 + 修复盘中 7 条缺 strength/track + 合并全量"""
import json, os, urllib.request, time

BASE = '/Users/jieyang/Documents/WealthHub'
NEWS = os.path.join(BASE, 'data/processed/news/news-2026-08-24.json')
SENT = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-24.json')
...
fixed = 0
for s in sents:
    if s.get('strength') in (None, ''):
        s['strength'] = s.get('score', 50)
        fixed += 1
    if not s.get('track'):
        for k, v in intraday_track_map.items():
            if s['title'].startswith(k):
                s['track'] = v
                break
        if not s.get('track'):
            s['track'] = '宏观'
        fixed += 1
print(f'修复盘中条目字段 {fixed} 处')

# -*- coding: utf-8 -*-
"""盘前档 2026-09-11: ①回填 9/10 遗留 3 条美股标普医药(XLV 9/10 -0.55%) ②新建 events-2026-09-11.json (18 条) + 历史事件库匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
NEWS_FILE = os.path.join(BASE, 'data/processed/news/news-2026-09-11.json')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-11.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-11.json')

# ---------- 0. 回填 9/10 遗留 3 条美股标普医药 (XLV 9/10 -0.55%) ----------
ev10_path = os.path.join(EVENTS_DIR, 'events-2026-09-10.json')
ev10 = json.load(open(ev10_path, encoding='utf-8'))
filled = []
for e in ev10:
    v = e.get('actual_ret_1d')
    if v is None:
        v = e.get('reference', {}).get('actual_ret_1d')
    if v is None and e['track'] == '美股标普医药':
        r = e.setdefault('reference', {})
        r['actual_ret_1d'] = -0.55
        r['actual_date'] = '2026-09-10'
        r['ret_1d_ref'] = 'XLV 9/10 -0.55%(收165.66, Lp(a)利空第3日跌幅仍温和)'
        filled.append(e['id'])
json.dump(ev10, open(ev10_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'回填 9/10 遗留美股标普医药: {filled} (XLV 9/10 -0.55%)')
...
print(f'news {len(news)} 条, sentiment {len(sents)} 条')

# ---------- 2. 历史事件库统计 (截至 9/10, 排除当日) ----------
all_events = []
...
# 校验 id 前缀 (§3.68 检查项 2)
assert all(e['id'].startswith('N20260911-') for e in new_events), 'id 前缀错误'
with open(OUT_FILE, 'w', encoding='utf-8') as f:
    json.dump(new_events, f, ensure_ascii=False, indent=1)
print(f"events-2026-09-11.json 新建 {len(new_events)} 条 (id 前缀校验通过)")
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']}/{e['strength']} | {e['title'][:38]}")

print("\n=== 历史事件 1日收益统计（按赛道, 截至 9/10 收盘回填） ===")
...
print(f"\n全库事件 {total} 条, 留空 {nulls} 条(9/11 当日 {len(new_events)} 条待收盘回填属正常)")

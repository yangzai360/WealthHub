# -*- coding: utf-8 -*-
"""盘前档 2026-08-31：组合估值重算（8/28 收盘真实净值全量）+ 赛道占比 + 集中度检查
基金净值从 fund_nav.csv 读取最新（QDII 8/27 已兑现，其余 8/28）；场内 ETF/个股用 8/28 收盘价"""
...
print("fund_nav 最新净值（QDII 8/27 已兑现、其余 8/28）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 8/28 收盘（盘前抓取 = 8/28 收盘价） ----------
ETF_SPOT = {}
try:
    ...
    print(f"[FAIL] fund_etf_spot_em: {e}（用 8/28 收盘价兜底）")
    ETF_SPOT = {"513050": 1.085, "159928": 0.672, "512170": 0.341, "513180": 0.584,
                "515180": 1.459, "159920": 1.483, "512880": 1.096, "512980": 0.809,
                "159938": 0.677}
for k, v in ETF_SPOT.items():
    print(f"  ETF {k}: {v}")

# ---------- 个股 8/28 收盘 ----------
STOCK_PRICE = {"002410": 9.00, "600438": 11.96}   # 广联达 9.00(+2.39%)、通威 11.96(+0.50%)
...
print("\n参考: 8/28 盘后重估总资产 388,583.22（组合 +0.04% 已确认）")
...
out = {"date": "2026-08-31", "as_of": "2026-08-28收盘", "snap_total": round(snap_total, 2),
       ...
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260831.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260831.json")

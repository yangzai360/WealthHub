# -*- coding: utf-8 -*-
"""2026-09-21 盘前: 链路累计/W39起累计/夏普比率/最大回撤（几何链式，§3.55/§3.91 口径）
本次为 32 个交易日样本；9/18 采用净值补更修正值（portfolio_close_20260918_fix.json）
"""
...
for p in sorted(glob.glob(os.path.join(HIST, 'portfolio_close_*.json'))):
    ...
    if pct is None:
        continue
    # 同日期后写覆盖（_fix 文件排序在后 → 生效），符合「修正优先」口径
    vals[date] = (float(pct), d.get('total_mv') or d.get('total'), d.get('med_pct'),
                  os.path.basename(p))
rows = sorted(vals.items())
rows = [(d, v[0], v[1], v[2], v[3]) for d, v in rows]
...
w = 1.0
wk = [(d, p) for d, p, *_ in rows if d >= '2026-09-14']
for d, p in wk:
    w *= (1 + p / 100)
print(f'上周 W38（9/14 起 {len(wk)} 日）累计: {(w-1)*100:.2f}%  ' + ' / '.join(f'{d[5:]} {p:+.2f}%' for d, p in wk))
...
json.dump(out, open(os.path.join(HIST, 'risk_stats_' + TODAY.replace('-', '') + '.json'), 'w', encoding='utf-8'),

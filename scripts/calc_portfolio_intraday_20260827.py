# -*- coding: utf-8 -*-
"""2026-08-27 盘中: 组合当日收益估算（8/26 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/27 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:44-13:45 抓取）
intraday = {
    # 场内ETF
    "513050": -0.18, "159928": 0.00, "512170": -0.29, "513180": -0.51,
    "159920": -0.46, "512880": 0.64, "512980": 0.37, "515180": 0.21,
    "159938": -0.15,
    # 指数
    "sh000001": 0.76, "sz399006": 0.70, "sh000932": 0.03,
    "HSTECH": -0.01, "HSI": -0.37,
    # 美股(8/25 XLV +0.34%, QDII 8/25 净值 +0.41% 今日 8/27-28 兑现, 盘中估算按 0, 兑现缓冲在报告单列)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/26 收盘口径市值 (portfolio_preopen_20260827.json detail)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260827.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 -0.22
...
def approx_pct(code, name):
    ...
    if code6 in ("000369", "016280"):
        return intraday["IYH"]          # 美股医药QDII -> IYH(8/25, 今日兑现按0)
    if code6 in ("000071",):
...
print(f"总资产(8/26收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/26收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260827.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260827.json")

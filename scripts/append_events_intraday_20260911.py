# -*- coding: utf-8 -*-
"""2026-09-11 盘中: 事件库追加 9 条盘中新闻 (id 019-027) + 历史匹配统计"""
import json, os, glob
...
# 盘中 9 条手动归类赛道（track 用赛道名，禁止 category 名；§3.64: 按『实际影响板块』归类）
TRACK_MAP = {
    "A股午评（9/11）": "宏观",
    "港股午评（9/11）": "恒生科技",
    "恒生科技13:45实时": "恒生科技",
    "中证消费13:45报12,365.69": "大消费",
    "创新药概念股持续调整（9/11）": "A股医药",
    "商保创新药目录过审率超93%": "A股医药",
    "工信部印发《『人工智能+软件』": "其他/宽基",
    "有色金属/铜价暴跌+锂业重挫": "宏观",
    "今晚20:30美国8月CPI": "宏观",
...
print(f"events-2026-09-11.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']} | {e['title'][:40]}")

print("\n=== 历史事件 1日收益统计（按赛道, 不含今日） ===")
for t, s in track_stats.items():
    print(f"  {t}: n={s['n']} avg={s['avg_ret_1d']} worst={s['worst_ret_1d']} best={s['best_ret_1d']} pos={s['pos_ratio']} ret3d={s['avg_ret_3d']} ret5d={s['avg_ret_5d']} ret10d={s['avg_ret_10d']} maxvol={s['max_vol']}")

tot = len(new_events)
pos = sum(1 for e in new_events if e['direction']=='利多')
neg = sum(1 for e in new_events if e['direction']=='利空')
neu = sum(1 for e in new_events if e['direction']=='中性')
scores = [e['score'] for e in new_events]
print(f"\n本次盘中 {tot} 条: 利多{pos} 利空{neg} 中性{neu}, 均值强度 {round(sum(scores)/len(scores)) if scores else 0}")

# 全库留空扫描
allp = sorted(glob.glob(os.path.join(EVENTS_DIR, 'events-*.json')))
total_ev = 0; blank = 0
for p in allp:
    with open(p, encoding='utf-8') as f:
        evs = json.load(f)
    for e in evs:
        ...
        if v is None:
            blank += 1
print(f"\n=== 全库留空扫描: 共 {total_ev} 条, 留空 {blank} 条 (今日新增 27 条待收盘回填) ===")

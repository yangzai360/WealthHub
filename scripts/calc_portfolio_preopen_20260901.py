# -*- coding: utf-8 -*-
"""盘前档 2026-09-01：组合估值重算（8/31 收盘真实净值全量，QDII 8/28 已兑现）+ 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 广发全球医疗 8/28 -0.38%/-0.35% 已兑现、其余 8/31）；场内 ETF/个股用 8/31 收盘价"""
...
print("fund_nav 最新净值（QDII 8/28 已兑现、其余 8/31）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 8/31 收盘（盘前抓取 fund_etf_spot_em 返回 8/31 收盘价） ----------
ETF_SPOT = {"513050": 1.078, "159928": 0.666, "512170": 0.339, "513180": 0.582,
            "515180": 1.468, "159920": 1.496, "512880": 1.105, "512980": 0.841,
            "159938": 0.643}
# ---------- 个股 8/31 收盘 ----------
STOCK_PRICE = {"002410": 8.99, "600438": 11.59}   # 广联达 8.99(-0.11%)、通威 11.59(-3.09%)
...
print("\n参考: 8/31 盘后重估总资产 387,658.66（8/28 收盘口径）")

# ---------- 夏普比率估算（十年国债收益率无风险基准） ----------
rf_10y = 1.69  # 中国10年期国债收益率 1.690%（9/1 早报）
daily_rets = []
...
out = {"date": "2026-09-01", "as_of": "2026-08-31收盘", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260901.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260901.json")

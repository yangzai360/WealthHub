# -*- coding: utf-8 -*-
"""2026-09-23 盘前档事件库处理：
  ① 回填：9/22 遗留的 73 条留空事件（= 9/22 盘前档 42 条 + 9/22 盘中档 31 条；9/22 盘后档未执行回填）
  ② 追加本档 39 条盘前事件（window == '盘前(9/22 18:00-9/23 07:30)'，含空集硬守卫）
  ③ 全库完整性扫描（留空按「日期×赛道」二维落库，§3.98）
  ④ 1日样本 + 3/5/10日窗口统计 → history/event_stats_20260923.json
⚠️ 口径问题：A股医药 3/5/10 日窗口序列实际使用 399006（创业板指）作代理（§3.90），报告须显式披露。
⚠️ 参考交易日规则（§3.95）：事件日之后的第一个交易日；美股标普医药用 XLV 序列（美东交易日）。
"""
...
by_evdate = defaultdict(int)
for x in back_detail:
    by_evdate[x['date']] += 1
print(f'\n回填 {backfilled} 条；按事件日 {dict(by_evdate)}')
...
for e in all_ev:
    r0 = e.get('reference', {})
    if r0.get('actual_date') != '2026-09-22':
        continue
    ...
    by_dir[d][0] += ok
    by_dir[d][1] += 1
print(f"\n方向验证（参考日 9/22）: {hit}/{tot}（{hit/tot*100:.1f}%）"
...
out = {
    ...
    'direction_check_0922': {'hit': hit, 'total': tot,
    ...
    'note': ('A股医药 3/5/10 日窗口序列实际使用 399006（创业板指）代理（§3.90）；'
             '本档回填 9/22 遗留 73 条（9/22 盘后档未执行回填，由本档兜底）；'
             '本档 39 条参考交易日=9/23（尚未收盘）→ 留空待 9/23 盘后档'),
}

# -*- coding: utf-8 -*-
"""盘中档 2026-09-21：且慢长赢(LONG_WIN) playwright 兜底数据落库（§3.82：mtime 最新 + 校验 composition）
   - plan 详情 → composition-2026-09-21.json + meta.json（含 nav_date_beijing）
...
# 上一档快照（9/20 周日档）作对照
prev_path = os.path.join(REF, 'composition-2026-09-20.json')
prev_pu, prev_cls = ({}, {})
if os.path.exists(prev_path):
    prev_pu, prev_cls = flatten_planunits(json.load(open(prev_path, encoding='utf-8')).get('composition'))
    print(f'对照快照 composition-2026-09-20.json 品种数={len(prev_pu)}  planUnit 合计={sum(v for v in prev_pu.values() if v)}')
else:
    print('⚠️ 无 9/20 对照快照')
...
print(f'  -> 源中不存在于本地（按北京日期直匹配）的日期: {sorted(new_rows) if new_rows else "无"}')

# -*- coding: utf-8 -*-
"""2026-09-17 盘中档：新浪 hq 直连「两次调用拿全」（§3.84 推荐做法）
request1: A股指数 + 港股指数 + 个股
request2: 场内ETF + A股板块指数
⚠️ CSV 追加必须按「末行 EOL 一致」的字节追加（§3.87 固化），禁止整表重写
"""
...
BOARDS = [("sh000933", "中证医药"), ("sz399989", "中证医疗"), ("sz399997", "中证白酒"),
          ("sh000827", "中证环保"), ("sh000922", "中证红利"), ("sh000934", "中证金融"),
          ("sh000949", "中证银行")]
...
if new_etf:
    eol = append_bytes(p_etf, new_etf, expect_bom=True)
print(f"etf_intraday.csv +{len(new_etf)} 行 (eol={eol!r})")
...
if new_idx:
    eol2 = append_bytes(p_idx, new_idx, expect_bom=True)
print(f"indices.csv +{len(new_idx)} 行（板块指数）(eol={eol2!r})")

json.dump(out, open(os.path.join(ROOT, "data/processed/history/intraday_hq_20260917.json"), "w",
                    encoding="utf-8"), ensure_ascii=False, indent=1)
print("已保存 intraday_hq_20260917.json")

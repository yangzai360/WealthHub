# -*- coding: utf-8 -*-
"""盘前档 2026-09-01:DeepSeek 情绪标注 9/1 盘前新闻(18条)"""
import json, urllib.request

NEWS_PATH = '/Users/jieyang/Documents/WealthHub/data/processed/news/news-2026-09-01.json'
SENT_PATH = '/Users/jieyang/Documents/WealthHub/data/processed/news/sentiment-2026-09-01.json'
...
{json.dumps(items, ensure_ascii=False)}"""
    ...
    if not content or '[' not in content:
        print(f'批次 {i//BATCH+1} content 为空,重试一次')
        content = call_deepseek(prompt)
...
assert len(all_results) == len(news), f'标注数量不匹配: {len(all_results)} vs {len(news)}'
...
for s in sentiments:
    print(f"  [{s['track']}] {s['sentiment']} {s['score']}/{s['strength']} {s['direction']} {s['volatility']} | {s['comment']}")

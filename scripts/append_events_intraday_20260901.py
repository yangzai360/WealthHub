# -*- coding: utf-8 -*-
"""2026-09-01 盘中: 事件库追加 8 条盘中新闻 (id 018-024) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-01.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-01.json')
...
TRACK_MAP = {
    "A股午评（9/1）": "宏观",
    "白酒「利空出尽」定价兑现（9/1盘中）": "大消费",
    "商务部7部门60万亿消费扩容方案今日定价（9/1盘中）": "大消费",
    "基药目录实施首日A股医药分化（9/1盘中）": "A股医药",
    "港股创新药续跌（9/1盘中）": "A股医药",
    "港股午评（9/1）": "恒生科技",
    "港股科技分化+猪价加速反弹（9/1午盘）": "恒生科技",
    "百济神州与美方达成PD-1降价协议（9/1）": "A股医药",
}
def get_track(title):
...
print(f"events-2026-09-01.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

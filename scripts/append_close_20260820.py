# -*- coding: utf-8 -*-
"""盘后档 2026-08-20 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(8/20当日净值+QDII补更)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260820.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股新闻口径补录（spot 连续第7日盘后失败，本次用新华社/证券时报收评：恒指 25698.49 +0.80% / 恒科 4700.53 +0.39%）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25698.49', '0.80', '收盘20:00(新闻口径补录)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4700.53', '0.39', '收盘20:00(新闻口径补录)'],
]
for h in hk_news:
    key = (h[1], h[3], h[6])
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= '2026-08-20':
        new_fund.append([TODAY, n['code'], n['name'], n['nav_date'], str(n['nav']), str(n['pct']) if n['pct'] is not None else ''])
# QDII 交银海外互联 8/19 补更（T+1，盘前停在 8/18）
extra_fund = [
    [TODAY, '164906', '交银海外互联', '2026-08-19', '0.962', '0.10'],
]

# -*- coding: utf-8 -*-
"""2026-09-15 盘后：抓取场外基金净值（天天基金 F10 直连，§3.79）+ 合并进 close_20260915.json"""
...
for code, name in funds:
    ...
    if d and d.get('Data') and d['Data'].get('LSJZList'):
        ...
        for r in rows:
            ...
            pr = r.get('JZZZL')
            try:
                pct = float(pr) if pr not in ('', None) else None
            except Exception:
                pct = None
            navs.append({'code': code, 'name': name, 'nav_date': nd, 'nav': nav, 'pct': pct})
...
print('\n净值日分布(>=9/11):', dict(c))
t15 = [n for n in navs if n['nav_date'] == '2026-09-15']
print(f'\n已出 9/15 净值 {len(t15)} 只:')
for n in sorted(t15, key=lambda x: -(x['pct'] or 0)):
    print(f"  {n['name']:20s} {n['code']} {n['nav']:.4f} {n['pct']:+.2f}%")
print(f'\nSAVED {OUT}  fund_navs={len(navs)}')

# -*- coding: utf-8 -*-
"""盘后档 2026-08-17：①回填 8/16 周末遗留 10 条事件 actual_ret_1d ②回填 8/17 当日 21 条
③追加盘后新增 8 条事件(id 022-029) ④美股标普医药留空待 8/18 ⑤样本统计"""
import json, os, glob
from collections import defaultdict, Counter

BASE = '/Users/jieyang/Documents/WealthHub'
EV = os.path.join(BASE, 'data/processed/events')
TODAY = '2026-08-17'

# ---------- 8/17 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': 0.66,       # 医疗ETF +0.57% / 医药ETF广发 +0.74% 均值（创新药强、药明A+H新高+4.80%）
    '大消费': -1.14,        # 中证消费 12,514.67 -1.14%（茅台失守1300 -3.64%、白酒板块-3.18%）
    '恒生科技': 1.58,       # HSTECH 4,782.03 +1.58%（重回4780上方、半导体暴涨）
    '宏观': 1.41,           # 上证指数 3,982.65 +1.41%（成交2.39万亿放量，社零弱但科技强）
    '其他/宽基': 0.16,      # 广联达 -0.55% / 通威 +0.86% 平均
    # 美股标普医药留空（8/17 美股未收盘，待 8/18 盘前用 XLV/IYH 回填）
}
def get_ret1d(e):
    v = e.get('actual_ret_1d')
    if v is None:
        v = e.get('reference', {}).get('actual_ret_1d')
    return v

# ---------- 1. 回填 8/16 周末遗留 10 条 ----------
ev16_path = os.path.join(EV, 'events-2026-08-16.json')
ev16 = json.load(open(ev16_path, encoding='utf-8'))
fix16 = 0
for x in ev16:
    r = x.get('reference') or {}
    if r.get('actual_ret_1d') is not None:
        continue
    if x.get('track') in track_ret:
        r['actual_ret_1d'] = track_ret[x['track']]
        r['actual_date'] = TODAY
        r['ret_1d_ref'] = '8/17收盘(周末事件首个交易日)'
        fix16 += 1
json.dump(ev16, open(ev16_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'8/16 遗留回填 {fix16} 条（8/17 收盘口径）')

# ---------- 2. 回填 8/17 当日 21 条 ----------
ev17_path = os.path.join(EV, 'events-2026-08-17.json')
ev17 = json.load(open(ev17_path, encoding='utf-8'))
fix17 = 0
for x in ev17:
    r = x.get('reference') or {}
    if r.get('actual_ret_1d') is not None:
        continue
    if x.get('track') in track_ret:
        r['actual_ret_1d'] = track_ret[x['track']]
        r['actual_date'] = TODAY
        r['ret_1d_ref'] = '赛道指数/ETF收盘'
        fix17 += 1
json.dump(ev17, open(ev17_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'8/17 当日回填 {fix17} 条；美股标普医药留空 {sum(1 for x in ev17 if x["track"]=="美股标普医药" and get_ret1d(x) is None)} 条待 8/18')

# ---------- 3. 追加盘后新增 8 条事件（id 022-029） ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-17.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev17)

TRACK_MAP = {
    '7月经济数据落地': '宏观',
    'A股收评：沪指+1.41%': '宏观',
    '茅台失守1,300元关口': '大消费',
    '药明康德A+H齐创历史新高：A股+4.80%': 'A股医药',
    '港股收评：恒指+1.34%': '恒生科技',
    '礼来retatrutide': '美股标普医药',
    "SK海力士董事长警告明年将现最严重'存储荒'": '恒生科技',
    '7月70城房价数据': '宏观',
}
def get_track(title):
    for k, v in TRACK_MAP.items():
        if title.startswith(k):
            return v
    return '宏观'

# 历史样本统计（供 reference）
all_events = []
for p in sorted(glob.glob(os.path.join(EV, 'events-*.json'))):
    if '2026-08-17' in p:
        continue
    all_events += json.load(open(p, encoding='utf-8'))
stats = defaultdict(lambda: {'n': 0, 'rets': [], 'pos': 0})
for e in all_events:
    v = get_ret1d(e)
    if v is None:
        continue
    t = e.get('track', '?')
    stats[t]['n'] += 1
    stats[t]['rets'].append(float(v))
    if float(v) > 0:
        stats[t]['pos'] += 1
track_stats = {}
for t, s in stats.items():
    track_stats[t] = {
        'n': s['n'],
        'avg': round(sum(s['rets'])/len(s['rets']), 2) if s['rets'] else None,
        'worst': round(min(s['rets']), 2) if s['rets'] else None,
        'best': round(max(s['rets']), 2) if s['rets'] else None,
        'pos': round(s['pos']/s['n'], 2) if s['n'] else None,
    }

new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-17.json'), encoding='utf-8'))
for n in news_all:
    if n['title'] in existing_titles:
        continue
    s = sent_by_title.get(n['title'], {})
    t = get_track(n['title'])
    st = track_stats.get(t, {})
    seq = len(ev17) + len(new_events) + 1
    new_events.append({
        'id': f'N{TODAY.replace("-", "")}-{seq:03d}',
        'date': TODAY,
        'track': t,
        'category': n.get('category', '行业事件类'),
        'title': n['title'],
        'summary': n.get('summary', ''),
        'source': n.get('source', 'WebSearch'),
        'source_url': n.get('source_url', ''),
        'sentiment': s.get('sentiment', '中性'),
        'score': s.get('score', 50),
        'strength': s.get('strength', s.get('score', 50)),
        'direction': s.get('direction', '中性'),
        'volatility': s.get('volatility', '中'),
        'reason': s.get('reason', ''),
        'reference': {
            'ret_3d': None, 'ret_5d': None, 'ret_10d': None,
            'max_vol': None,
            'confidence': min(90, 40 + st.get('n', 0) * 2),
            'actual_ret_1d': None,
            'actual_date': None,
            'ret_1d_ref': {
                'track_n': st.get('n'), 'track_avg': st.get('avg'),
                'track_worst': st.get('worst'), 'track_best': st.get('best'),
                'track_pos_ratio': st.get('pos'),
            }
        }
    })

combined = ev17 + new_events
json.dump(combined, open(ev17_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-17.json 共 {len(combined)} 条')
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']}/{e['strength']} | {e['title'][:40]}")

# ---------- 4. 全量样本统计 ----------
all_ev = []
for f in sorted(glob.glob(os.path.join(EV, 'events-*.json'))):
    all_ev += json.load(open(f, encoding='utf-8'))
print('\n事件库全量累计:', len(all_ev), '条')
final = defaultdict(lambda: {'n': 0, 'rets': [], 'sp': {'n': 0, 'rets': []}, 'sn': {'n': 0, 'rets': []}})
for x in all_ev:
    v = get_ret1d(x)
    if v is None:
        continue
    t = x.get('track', '?'); s = final[t]
    s['n'] += 1; s['rets'].append(float(v))
    strength = x.get('strength') or x.get('score') or 0
    if strength >= 60 and x.get('sentiment') == '正面':
        s['sp']['n'] += 1; s['sp']['rets'].append(float(v))
    if strength >= 60 and x.get('sentiment') == '负面':
        s['sn']['n'] += 1; s['sn']['rets'].append(float(v))
print('\n赛道 1 日样本统计（含 8/16 回填 + 8/17）：')
for t, s in sorted(final.items()):
    def avg(lst): return round(sum(lst)/len(lst), 2) if lst else None
    print(f"  {t}: n={s['n']} 均值={avg(s['rets'])} | 强正面 n={s['sp']['n']} 均值={avg(s['sp']['rets'])} | 强负面 n={s['sn']['n']} 均值={avg(s['sn']['rets'])}")

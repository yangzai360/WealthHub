# -*- coding: utf-8 -*-
"""盘后档 2026-08-21 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(8/20当日净值+QDII补更)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260821.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股新闻口径补录（spot 连续第8日盘后失败，本次用新浪财经/证券时报/每经收评多源交叉：恒指 26009.46 +1.21% / 恒科 4766.16 +1.40%）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '26009.46', '1.21', '收盘20:00(新闻口径补录)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4766.16', '1.40', '收盘20:00(新闻口径补录)'],
]
for h in hk_news:
    key = (h[1], h[3], h[6])
...
# ---------- 3. fund_nav (code, nav_date) 判重，8/20 当日净值 + QDII 补更 ----------
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= '2026-08-21':
        new_fund.append([TODAY, n['code'], n['name'], n['nav_date'], str(n['nav']), str(n['pct']) if n['pct'] is not None else ''])
# QDII 补更（8/20 净值，T+1 兑现 8/20 美股走势）：广发全球医疗A 8/20 -1.08%（对应 8/20 美股医疗 -1.87%）/ 交银海外互联 8/20 -1.12%（对应 8/20 中概互联 -1.06%）
extra_fund = [
    [TODAY, '000369', '广发全球医疗A', '2026-08-20', '2.651', '-1.08'],
    [TODAY, '164906', '交银海外互联', '2026-08-20', '0.9512', '-1.12'],
]
...
if new_fund:
    ...
    print('fund_nav 无 8/20 当日新增（T+1 未更新，正常）')

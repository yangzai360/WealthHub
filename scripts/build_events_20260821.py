# -*- coding: utf-8 -*-
"""盘前档 2026-08-21 事件库构建：从 sentiment 转 events（含 id/reference/strength）+ 历史样本统计"""
import json, os, glob
...
for track in ['A股医药', '大消费', '恒生科技', '美股标普医药', '宏观']:
    ...
    if rets:
        ...
        stats[track] = {'n': len(rets), 'avg': round(avg, 2), 'worst': round(worst, 2),
                        'best': round(best, 2), 'pos': round(pos, 0)}
        print(f'  {track}: 样本{len(rets)}条 1日均值 {avg:+.2f}% 最差 {worst:+.2f}% 最好 {best:+.2f}% 正收益占比 {pos:.0f}%')
    else:
...
with open(os.path.join(BASE, 'data/processed/history/event_stats_20260821.json'), 'w', encoding='utf-8') as f:
...
print('已保存 event_stats_20260821.json')

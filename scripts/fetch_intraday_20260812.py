# -*- coding: utf-8 -*-
"""
盘中行情抓取 (2026-08-12 13:45 档)
源: akshare 新浪源 + 天天基金源 (东财 push2 已被代理拦截, 不适用)
输出: data/processed/history/{indices,etf_intraday,fund_nav}.csv (增量) + intraday_20260812.json
"""
...
def append_unique(path, rows, key_idx, header):
    """增量追加: 用指定列索引组合判重, 同 key 保留旧行(盘中不覆盖收盘/盘前)"""
...
# ---------- 写入 (增量, 盘中不覆盖盘前/收盘; fund_nav 用 code+nav_date 判重) ----------
...
# 汇总 JSON
with open(os.path.join(HIST, "intraday_20260812.json"), "w", encoding="utf-8") as f:

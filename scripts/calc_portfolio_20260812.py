# -*- coding: utf-8 -*-
"""持仓合并 + 赛道占比 + 盘中组合估算收益 (2026-08-12 13:45 档)"""
...
# 实时: 医药ETF -0.29 / 医疗ETF -0.28; 消费ETF 0.00 / 中证消费 +0.01
# 恒科: HSTECH -1.37 / 恒指科技ETF -1.31 / 中概互联 -2.41 / 恒生ETF -1.26
# A股: 上证 +0.16 / 创业板 +1.19
PROXY = {
    "A股医药": -0.29,        # 医药ETF -0.29 / 医疗ETF -0.28 均值 (场外 T+1 仍显示 8/11)
    "大消费": 0.00,          # 消费ETF 0.00 / 中证消费 +0.01
    "美股标普医药": 0.0,     # QDII T+1, 8/10 已反映(近似 0)
    "恒生科技": -1.66,       # HSTECH -1.37 / 恒指科技 -1.31 / 中概互联 -2.41 均值
    "其他/宽基": 0.16,       # 上证 +0.16 (宽基近似)
    "现金": 0.0,
}
ETF_REALTIME = {
    "159938": -0.29, "512170": -0.28, "159928": 0.00, "513180": -1.31,
    "513050": -2.41, "159920": -1.26, "512880": 0.00, "512980": 0.80,
    "515180": -0.42,
}
# 个股盘中价暂缺, 用指数近似: 广联达(软件)→创业板指 +1.19, 通威(光伏)→上证 +0.16 保守
STOCK_PROXY = {"002410": 1.19, "600438": 0.16}
def est_pct(row):
...
with open(os.path.join(BASE, "data/processed/history/portfolio_20260812.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),

# -*- coding: utf-8 -*-
"""2026-09-18 盘后：抓取收盘行情（新浪 hq 直连，两次调用拿全，§3.84/§3.87 推荐做法）
注意 §3.87/§3.92：不请求 sh000922（中证红利）/ sh000949（中证银行，第 2 例空值），
红利方向用持仓 sh515180 代理；pct() 加零值守卫，写入前剔除「现价=0/名称空」条目。
板块指数沿用 9/18 盘中已验证可用集合（sh000933/sh000913/sz399975/sh000934/sh000827）。
"""

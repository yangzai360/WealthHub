# -*- coding: utf-8 -*-
"""盘后档 2026-08-18：①回填 8/17 遗留美股标普医药 3 条(XLV 8/17 收盘) ②回填 8/18 当日 29 条
③追加盘后新增 7 条事件(id 030-036) ④美股标普医药留空待 8/19 ⑤样本统计"""
import json, os, glob
...
# ---------- 8/18 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': -0.22,       # 医疗ETF 0.00% / 医药ETF广发 -0.44% 均值（药明A +0.17% 高位震荡、港股CXO强）
    '大消费': 0.89,         # 中证消费 12,626.01 +0.89%（站稳12600，茅台尾盘翻红 +0.38% 收1297.99）
    '恒生科技': -0.90,       # HSTECH 4,739.18 -0.90%（盘中曾跌2.2%、尾盘收复4700；阿里+3.68% vs 智谱-13.3%）
    '宏观': 0.19,            # 上证指数 3,990.30 +0.19%（冲4000失败，第二日）
    '其他/宽基': -0.78,      # 广联达 -0.55% / 通威 -1.01% 平均
    # 美股标普医药留空（8/18 美股未收盘，待 8/19 盘前用 XLV/IYH 回填）
}
def get_ret1d(e):
...
# ---------- 1. 回填 8/17 遗留美股标普医药 3 条（XLV 8/17 收盘 -0.19%，两标的方向一致取 XLV 口径） ----------
...
for x in ev17:
    ...
    if r.get('actual_ret_1d') is not None:
        continue
    if x.get('track') == '美股标普医药':
        r['actual_ret_1d'] = -0.19
        r['actual_date'] = '2026-08-17'
        r['ret_1d_ref'] = 'XLV 8/17收盘(-0.19%)'
        fix17 += 1
json.dump(ev17, open(ev17_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'8/17 美股标普医药遗留回填 {fix17} 条（XLV 8/17 收盘口径）')
...
print(f'8/18 当日回填 {fix18} 条；美股标普医药留空 {sum(1 for x in ev18 if x["track"]=="美股标普医药" and get_ret1d(x) is None)} 条待 8/19')

# ---------- 3. 追加盘后新增 7 条事件（id 030-036） ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-18.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev18)
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-18.json'), encoding='utf-8'))
for n in news_all:
...
combined = ev18 + new_events
json.dump(combined, open(ev18_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-18.json 共 {len(combined)} 条')
for e in new_events:

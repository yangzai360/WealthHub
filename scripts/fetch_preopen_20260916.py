# -*- coding: utf-8 -*-
"""盘前档 2026-09-16(周三) 行情抓取:
1) 场外基金净值增量 —— 用天天基金 F10 直连(§3.79 推荐, 秒级返回); 9/15 A股类已盘后全量落地, 本次主要补 3 只 T+1(164906/000369/016280)
2) 美股(美东 9/15 周二收盘 = 隔夜, FOMC 首日)
3) 港股 hq 直连(带 Referer, 9/15 收盘) + A股指数日线(9/15 收盘) + 个股
4) 且慢 pmdj 复测
"""
...
with open(csv_path, encoding='utf-8-sig') as f:
    for row in csv.DictReader(f):
        # ⚠️ 判重 key 必须为 (code, nav_date) 二元组，不可含「写入日」（§3.86 教训）

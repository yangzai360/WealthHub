# -*- coding: utf-8 -*-
"""2026-09-20 周日盘后档：DeepSeek v4-flash 情绪标注（周末窗口 9/18 20:00 - 9/20 20:00，24 条）
⚠️ §3.88：必须为每条 item 写 `window` 字段（文件级字段不可替代）；过滤后为空须硬报错。
§3.93：max_tokens 必须 8000+（思考占满 token 会导致 content 为空）
"""
import json, os, time, urllib.request

BASE = '/Users/jieyang/Documents/WealthHub'
NEWS = os.path.join(BASE, 'data/processed/news/news-2026-09-20.json')
SENT = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-20.json')
WINDOW = '周末(9/18 20:00-9/20 20:00)'

with open('/Users/jieyang/.pi/agent/auth.json') as f:
    KEY = json.load(f)['deepseek']['key']

news = json.load(open(NEWS, encoding='utf-8'))
sent = {'date': '2026-09-20', 'window': WINDOW, 'items': []}
...
for k, (n, a) in enumerate(results, start=1):
...
# ⚠️ §3.88 硬守卫
if not sent['items']:
    raise SystemExit('⚠️ 周末窗口 sentiment 为 0 条，终止')
if any(x.get('window') != WINDOW for x in sent['items']):
    raise SystemExit('⚠️ 存在缺失/错误 window 字段，终止')

np_ = sum(1 for x in sent['items'] if x['direction'] == '利多')
nn_ = sum(1 for x in sent['items'] if x['direction'] == '利空')
nu_ = sum(1 for x in sent['items'] if x['direction'] == '中性')
print(f"\n周末 {len(sent['items'])} 条：利多 {np_} / 利空 {nn_} / 中性 {nu_}，"
      f"均值强度 {sum(x['strength'] for x in sent['items']) / len(sent['items']):.1f}")
for x in sent['items']:

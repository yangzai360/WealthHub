# -*- coding: utf-8 -*-
"""2026-08-24 盘中: 组合当日收益估算（8/21 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/24 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:41-13:45 抓取）
intraday = {
    # 场内ETF
    "513050": -4.29, "159928": 0.15, "512170": -3.15, "513180": -3.34,
    "159920": -1.77, "512880": 0.19, "512980": -2.35, "515180": 1.05,
    "159938": -3.32,
    # 指数
    "sh000001": -1.07, "sz399006": -4.34, "sh000932": 0.17,
    "HSTECH": -3.68, "HSI": -1.98,
    # 美股(8/21收盘 +1.29%, QDII 8/25-26 才兑现, 今日盘中按 0)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/21 收盘口径市值 (portfolio_preopen_20260824.json detail)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260824.json"), encoding="utf-8") as f:
    preopen = json.load(f)
...
def approx_pct(code, name):
    ...
    if code6 in ("000369", "016280"):
        return intraday["IYH"]          # 美股医药QDII -> IYH(8/21, 今日按0)
    if code6 in ("000071",):
...
print(f"总资产(8/21收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/21收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260824.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260824.json")

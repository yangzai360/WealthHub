# -*- coding: utf-8 -*-
"""盘前档 2026-09-21(周一) 行情抓取:
1) 场外基金净值增量 —— 天天基金 F10 直连(§3.79); 判重 key = (code, nav_date) 二元组(§3.86)
   fund_nav.csv 现状(实测): 无 BOM + 全 LF + 末行 LF -> 直接字节追加 '\n'
   重点: 复核 QDII/LOF 三只 (000369/016280/164906) 的 9/18 净值是否出库
2) 美股(美东 9/18 周五收盘 = 隔夜) —— 复核是否已归档
3) 港股 hq 直连(9/18 收盘) + A股指数日线(9/18 收盘) + 个股
4) 且慢 pmdj 复测

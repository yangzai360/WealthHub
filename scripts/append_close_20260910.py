# -*- coding: utf-8 -*-
"""2026-09-10 盘后: close_20260910.json -> indices.csv/etf_intraday.csv/fund_nav.csv 增量写入
...
# 3. fund_nav（写 nav_date>=2026-09-09：覆盖 QDII T+1 补更 + 当日 9/10；判重 [code,nav_date]）
nav_rows = [['2026-09-10', r['code'], r['name'], r['nav_date'], r['nav'], r['pct'] if r['pct'] is not None else '']
            for r in close['fund_navs'] if r['nav_date'] >= '2026-09-09']
n3 = append_unique(os.path.join(HIST, 'fund_nav.csv'), nav_rows, [1, 3],
                   'date,code,name,nav_date,nav,pct')
print(f'[fund_nav] 新增 {n3} 条 (nav_date>=9/9)')

# 校验: 当日 indices 含 A股 5 + HSI/HSTECH 收盘行 (§3.59/§3.62 教训)
with open(os.path.join(HIST, 'indices.csv'), encoding='utf-8-sig') as f:
    lines = f.readlines()
today_rows = [l.strip() for l in lines if '2026-09-10' in l and '收盘20:00' in l]
...
with open(os.path.join(HIST, 'fund_nav.csv'), encoding='utf-8-sig') as f:
    fl = f.readlines()
fn_rows = [l.strip() for l in fl if ',2026-09-10,' in l]
print(f'\n校验 fund_nav.csv 当日(9/10)净值行 {len(fn_rows)} 条')
for l in fn_rows:
    print('  ' + l)
print('\n校验通过: A股5 + 港股2 收盘行齐全')

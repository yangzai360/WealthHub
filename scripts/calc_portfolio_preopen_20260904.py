# -*- coding: utf-8 -*-
"""盘前档 2026-09-04：组合估值重算（9/2 收盘真实净值全量，QDII 广发全球医疗 9/1 +0.54%/+0.55% 已兑现、交银海外互联 9/1 -1.13%）+ 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 9/1、其余 9/2）；场内 ETF/个股用 9/2 收盘价"""
...
print("fund_nav 最新净值（QDII 9/2 兑现、其余 9/3）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 9/2 收盘（etf_intraday.csv 收盘20:00 口径） ----------
ETF_SPOT = {"513050": 1.035, "159928": 0.665, "512170": 0.339, "513180": 0.565,
            "515180": 1.450, "159920": 1.481, "512880": 1.098, "512980": 0.826,
            "159938": 0.644}
# ---------- 个股 9/2 收盘 ----------
STOCK_PRICE = {"002410": 8.60, "600438": 11.28}   # 广联达 8.60(-0.92%)、通威 11.28(-0.70%)
...
print(f"\n9/3 收盘口径总资产(净值全量修正): {total_mv:,.2f}")
...
print("\n参考: 9/3 盘后代理口径 382,688.59（QDII 停 9/1）；本次为 QDII 9/2 兑现 + 场外 9/3 净值全量修正口径")
...
out = {"date": "2026-09-04", "as_of": "2026-09-03收盘(净值全量修正)", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260904.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260904.json")

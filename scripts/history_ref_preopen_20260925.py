# -*- coding: utf-8 -*-
"""2026-09-25 盘前档：历史相似事件匹配（品类+赛道+关键词 Jaccard → Top3）→ 3/5/10 日窗口
...
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-09-25.json'), encoding='utf-8'))
news = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-09-25.json'), encoding='utf-8'))
todays = [n for n in news if n.get('window') == WIN]
print(f'全库 {len(allv)} 条；本档待匹配 {len(todays)} 条（窗口 {WIN}）')
if len(todays) == 0:
    raise SystemExit('⚠️ §3.103 硬守卫：按 window 过滤 news 得 0 条 → window 字段可能未写入 news-*.json，终止以免静默空跑')
assert sum(1 for n in news if n.get('window') == WIN) > 0, '§3.103 断言失败：news 缺 window 字段'
...
OUTFILE = f"history_ref_{TODAY.replace('-', '')}_preopen.json"
json.dump(out, open(os.path.join(HIST, OUTFILE), 'w', encoding='utf-8'),

# -*- coding: utf-8 -*-
"""盘后档 2026-08-17 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(8/17当日净值)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260817.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股新闻口径补录（spot 失败，本次用新华社收评：恒指 25453.23 +1.34% / 恒科 4782.03 +1.58%）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25453.23', '1.34', '收盘20:00(新闻口径补录)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4782.03', '1.58', '收盘20:00(新闻口径补录)'],
]
for h in hk_news:
    key = (h[1], h[3], h[6])
...
# ---------- 3. fund_nav (code, nav_date) 判重，8/17 当日净值（本次仅富国消费主题已出 8/17） ----------
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= '2026-08-17':
...
if new_fund:
    with open(fund_path, 'a', encoding='utf-8') as f:
        for row in new_fund:
            f.write(','.join(row) + '\n')
    print(f'\nfund_nav 新增 {len(new_fund)} 行(8/17当日):')
    for r in new_fund:
        print('  ' + ','.join(r))
else:
    print('fund_nav 无 8/17 当日新增（T+1 未更新，正常）')

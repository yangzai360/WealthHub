# -*- coding: utf-8 -*-
"""2026-09-24 盘后: 链路累计/W39起累计/夏普比率/最大回撤（几何链式，§3.55/§3.91/§3.95/§3.98 口径）
同日期以 `_fix` 为准 —— 显式优先级判断（禁止依赖 glob/sorted 顺序，§3.98）
⚠️ 本档新增修复：`est_total_pct_raw` 字段**存在但值为 None** 时，`.get(k, default)` 会返回 None 而非 default
   → 原脚本会静默跳过 `_fix` 文件（如 portfolio_close_20260922_fix.json raw=None、pct=0.1396）
   → 改用显式 None 判断（§3.102）
"""
...
print(f'连续性抽查：9/23 盘前档 chain=-2.55% / 34 日 → 本档 {out["chain_cum_pct"]:.2f}% / {out["chain_days"]} 日，'
      f'差分 {out["chain_cum_pct"]-(-2.55):+.2f}pct（应≈当日日收益 {rets[-1]:+.2f}%）')

# -*- coding: utf-8 -*-
"""盘后档 2026-08-14：组合真实收盘收益重算 + 赛道归因"""
...
# ---------- 8/14 真实收盘涨跌幅 ----------
# 场内 ETF 收盘（close_20260814.json）
ETF_CLOSE = {
    "159938": -1.02, "512170": -1.13, "159928": -1.61, "513180": -1.49,
    "513050": -1.88, "159920": -0.87, "512880": -1.27, "512980": 0.12,
    "515180": 0.21,
}
# 场外基金 8/14 当日净值（已出）
FUND_NAV_814 = {
    "519915": -0.37, "012348": -1.71, "012323": -1.03,
}
# QDII 8/13 补更（昨日记 0，今日补更，标注 T+1）
FUND_NAV_813_UPDATE = {
    "164906": -1.75, "016280": -0.04,
}
# 个股真实收盘
STOCK_CLOSE = {"002410": -1.29, "600438": -0.70}
def est_pct(row):
    code6 = re.sub(r"\D", "", str(row["code"]) if pd.notna(row["code"]) else "")
    if code6 in ETF_CLOSE:
        return ETF_CLOSE[code6]
    if code6 in FUND_NAV_814:
        return FUND_NAV_814[code6]
    if code6 in FUND_NAV_813_UPDATE:
        return FUND_NAV_813_UPDATE[code6]
...
with open(os.path.join(BASE, "data/processed/history/portfolio_close_20260814.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),
...
print("\n已保存 portfolio_close_20260814.json")

# -*- coding: utf-8 -*-
"""盘后档 2026-08-26：①回填 8/26 当日 24 条（美股标普医药 3 条留空待 8/27）
...
# ---------- 8/26 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': -0.22,        # ETF均值（医疗ETF -0.58% / 医药ETF广发 +0.15%）；医疗服务 -1.34% 领跌、创新药偏强分化
    '大消费': -0.06,         # 中证消费 12,484.34 -0.06% 守住12,400（茅台 1,302.80 -0.09% 守1,300、泸州老窖 -2.37% 软着陆、白酒板块 +0.64%）
    '恒生科技': 0.82,        # HSTECH 4,626.15 +0.82%（恒指 25,652.97 +0.56%、国企指数 +1.05%；券商/有色/地产领涨）
    '宏观': 0.59,            # 上证指数 3,912.52 +0.59%（创业板 +0.51%、科创50 +1.71% 领涨、深成指 +0.69%）
    '其他/宽基': 1.05,       # 广联达 +2.09% / 通威 0.00% 平均
    # 美股标普医药留空（8/26 美股未收盘，待 8/27 用 XLV/IYH 回填）
}
def get_ret1d(e):
...
# ---------- 1. 回填 8/26 当日 24 条 ----------
ev26_path = os.path.join(EV, 'events-2026-08-26.json')
...
print(f'8/26 当日回填 {fix26} 条；留空 {len(left26)} 条（美股标普医药待 8/27）')
for x in left26:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

# ---------- 2. 追加盘后 8 条事件（id 025-032）+ 回填 ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-26.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev26)

# 历史样本统计（供 reference，不含 8/26）
all_events = []
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-26.json'), encoding='utf-8'))
for n in news_all:
...
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-26.json 共 {len(combined)} 条')
for e in new_events:

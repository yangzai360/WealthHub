# -*- coding: utf-8 -*-
"""2026-09-10 盘中: 个股实时(sina hq 直连) + 港股/指数补充 + 且慢 E大调仓监控"""
import urllib.request, json, os, re, time

BASE = "/Users/jieyang/Documents/WealthHub"

def fetch(url, encoding="utf-8", timeout=25):
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read().decode(encoding, errors="ignore")

print("=== 1. 个股实时 (sina hq 直连, GBK) ===")
try:
    raw = fetch("http://hq.sinajs.cn/list=sz002410,sh600438,sh600519,sh000001,sz399006,sh000932,sh000300", encoding="gbk")

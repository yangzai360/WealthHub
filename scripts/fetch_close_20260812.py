# -*- coding: utf-8 -*-
"""盘后档 2026-08-12 收盘行情抓取：A股指数日线 / 港股 spot / 场内ETF / 个股 / 场外净值 / 美股(XLV/IYH 8/11回填) / 且慢pmdj复测"""
import json, os, sys, time, re, csv, urllib.request
                    ...
                    if code in str(row.get('code', '')) or (code == 'HSI' and '恒生指数' in nm) or (code == 'HSTECH' and '恒生科技' in nm):
                        if code not in hk_done:
                            hk_done[code] = {'type': 'index', 'date': TODAY, 'name': cname, 'code': code,
                                             'close': round(float(row['latest']), 2),
                                             'pct_change': round(float(row['changepercent']), 2), 'note': '收盘20:00'}
...
for code in hk_map:
    if code not in hk_done:
        print(f'  WARN 港股 {hk_map[code]} 数据暂缺')
...
# ---------- 汇总输出 ----------
out = {'date': TODAY, 'indices': indices, 'hk': list(hk_done.values()), 'etf': etfs, 'stocks': stocks, 'fund_navs': fund_navs, 'us': us, 'qieman': qieman}
with open(os.path.join(HIST, 'close_20260812.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> close_20260812.json')

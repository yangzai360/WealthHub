# -*- coding: utf-8 -*-
"""盘前档 2026-09-09：组合估值重算（9/8 收盘真实净值全量口径，医药主动基 9/8 大涨兑现 + QDII 停 9/7） + 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 9/7、其余 9/8）；场内 ETF/个股用 9/8 收盘价"""
...
print("fund_nav 最新净值（QDII 9/7、其余 9/8）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 9/8 收盘（etf_intraday.csv 收盘20:00 口径） ----------
ETF_SPOT = {}
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-08" and "收盘" in row.get("note", ""):
...
# ---------- 个股 9/8 收盘 ----------
STOCK_PRICE = {"002410": 8.87, "600438": 11.86}   # 广联达 8.87(+0.68%)、通威 11.86(+1.63%)
...
print(f"\n9/8 收盘口径总资产(净值全量修正): {total_mv:,.2f}")
...
print("\n参考: 9/8 盘后代理口径 385,309.86；本次为场外 9/8 净值全量(医药主动基大涨)修正口径")
...
out = {"date": "2026-09-09", "as_of": "2026-09-08收盘(净值全量修正,QDII 停9/7)", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260909.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260909.json")

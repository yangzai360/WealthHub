# -*- coding: utf-8 -*-
"""盘前档 2026-09-03：组合估值重算（9/2 收盘真实净值全量，QDII 广发全球医疗 9/1 +0.54%/+0.55% 已兑现、交银海外互联 9/1 -1.13%）+ 赛道占比 + 集中度检查 + 夏普比率估算
...
print("fund_nav 最新净值（QDII 9/1 兑现、其余 9/2）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 9/2 收盘（etf_intraday.csv 收盘20:00 口径） ----------
ETF_SPOT = {"513050": 1.050, "159928": 0.668, "512170": 0.338, "513180": 0.569,
            "515180": 1.448, "159920": 1.483, "512880": 1.090, "512980": 0.830,
            "159938": 0.641}
# ---------- 个股 9/2 收盘 ----------
STOCK_PRICE = {"002410": 8.68, "600438": 11.36}   # 广联达 8.68(-2.58%)、通威 11.36(-3.89%)
...
print("\n参考: 9/2 盘后重估总资产 382,317.65（QDII 停 8/31 口径）；本次为 QDII 9/1 兑现 + 场外 9/2 净值全量修正口径")
...
out = {"date": "2026-09-03", "as_of": "2026-09-02收盘", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260903.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260903.json")

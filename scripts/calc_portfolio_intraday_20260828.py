# -*- coding: utf-8 -*-
"""2026-08-28 盘中: 组合当日收益估算（8/27 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/28 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:45 抓取）
intraday = {
    # 场内ETF
    "513050": 0.19, "159928": 0.45, "512170": -1.16, "513180": 0.00,
    "159920": 0.27, "512880": -0.09, "512980": 0.00, "515180": 0.27,
    "159938": -1.06,
    # 指数
    "sh000001": 0.01, "sz399006": -1.00, "sh000932": 0.25,
    "HSTECH": 0.13, "HSI": 0.30,
    # 美股(8/27 XLV -1.13% 于 8/28-29 兑现, 盘中估算按 0, 兑现压力在报告说明)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/27 收盘口径市值 (portfolio_preopen_20260828.json detail)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260828.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 -1.11
...
print(f"总资产(8/27收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/27收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260828.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260828.json")

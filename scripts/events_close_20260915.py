# -*- coding: utf-8 -*-
"""2026-09-15 盘后档事件库处理：
  ① 追加盘后 18 条事件（N20260915-056~073）
  ② 回填 9/15 当日 73 条 actual_ret_1d（美股标普医药留空，待 9/16 盘前档兜底）
  ③ 全库完整性扫描
  ④ 1日样本 + 方向验证 + 3/5/10日窗口统计 → history/event_stats_20260915.json（收盘口径覆盖）
统计口径与 scripts/event_stats_20260915.py 保持一致（sentiment 口径 + track_index 代理序列）
"""
...
track_ret = {
    'A股医药': -0.78,    # ETF均值（医药ETF广发 -0.95% / 医疗ETF -0.60%）；申万医药生物 -1.07%
    '大消费': -0.57,     # 中证消费 12,253.35 -0.57%（跌破 12,300）
    '恒生科技': -0.62,   # HSTECH 4,291.34 -0.62%
    '宏观': -0.54,       # 上证指数 3,864.28 -0.54%
    '其他/宽基': -0.29,  # 个股均值（广联达 +0.48% / 通威 -1.06%）
    # 美股标普医药：9/15 美股未收盘 → 留空待 9/16 盘前档兜底（§3.70）
}
REF = {
    'A股医药': '9/15 收盘：医药ETF广发 -0.95% / 医疗ETF -0.60%（申万医药生物 -1.07%）',
    '大消费': '9/15 收盘：中证消费 12,253.35 -0.57%',
    '恒生科技': '9/15 收盘：HSTECH 4,291.34 -0.62%',
    '宏观': '9/15 收盘：上证指数 3,864.28 -0.54%',
    '其他/宽基': '9/15 收盘：个股均值（广联达 +0.48% / 通威 -1.06%）',
}
...
print(f'回填 {filled} 条；留空 {blank} 条（美股标普医药，待 9/16 盘前档按 XLV 9/15 收盘兜底）')
...
# ---------- 3/5/10 日窗口（沿用盘前档 track_index 代理序列） ----------
...
out = {'date': TODAY, 'as_of': '2026-09-15收盘回填', 'library_total': len(all_ev),

# -*- coding: utf-8 -*-
"""盘后档 2026-09-01 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(9/1当日已出)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260901.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股收盘补录（spot 盘后连续第15日失败；新浪 hq 接口直取 9/1 收盘：恒指 25329.73 -0.93% / 恒科 4550.88 -1.49% 守4,500）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25329.73', '-0.93', '收盘20:00(新浪hq直取)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4550.88', '-1.49', '收盘20:00(新浪hq直取)'],
]
if new_idx:
...
# ---------- 3. fund_nav (code, nav_date) 判重：9/1 当日已出净值 + QDII 8/28 已归档 ----------
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= TODAY:
        # 仅写 9/1 当日净值（8/31 及更早已在盘前归档；QDII 广发全球医疗 8/28 已兑现）

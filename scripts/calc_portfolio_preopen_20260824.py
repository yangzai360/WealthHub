# -*- coding: utf-8 -*-
"""盘前档 2026-08-24：组合估值重算（8/21 收盘真实净值全量 + 场内 ETF 实时价）+ 赛道占比 + 集中度检查"""
...
# ---------- 8/21 收盘真实净值（今日盘前抓取全量；QDII 用 8/20 最新 T+1~T+2） ----------
FUND_NAV_821 = {
    "002708": 2.001, "000968": 0.8377, "002742": 1.2602, "004752": 0.8104,
    "005368": 1.1505, "110020": 1.8898, "100032": 0.976, "001180": 0.8267,
    "161616": 1.845, "000051": 1.7638, "519915": 1.857, "000071": 1.5354,
    "012348": 0.6508, "001551": 0.7599, "000248": 1.8132,
    "001469": 1.2256, "001552": 1.0263, "012323": 0.6008,
    "000727": 2.558, "004424": 1.7812,
    "000369": 2.651, "016280": 2.607, "164906": 0.9512,   # QDII 8/20（8/20 美股 -1.89% 已兑现 -1.08%/-1.06%；8/21 美股 +1.29% 待 8/25-26 兑现）
}
# 场内 ETF 8/21 收盘价（fund_etf_spot_em 抓取 = 最后收盘价）
ETF_SPOT = {}
try:
    ...
    print(f"[FAIL] fund_etf_spot_em: {e}（用 8/20 收盘价兜底）")
    ETF_SPOT = {"513050": 1.131, "159928": 0.669, "512170": 0.359, "513180": 0.597,
                "515180": 1.434, "159920": 1.516, "512880": 1.063, "512980": 0.813,
                "159938": 0.687}
for k, v in ETF_SPOT.items():
    print(f"  ETF {k}: {v}")

# 个股 8/21 收盘（close_20260821 实测口径）
STOCK_PRICE_821 = {"002410": 9.01, "600438": 12.43}   # 广联达 9.01(MA60 9.03 下方)、通威 12.43(+0.89%)
...
def calc_market_value(row):
    ...
    if code6 in STOCK_PRICE_821:
        return shares * STOCK_PRICE_821[code6], row["amount"]
    if code6 in FUND_NAV_821:
        return shares * FUND_NAV_821[code6], row["amount"]
...
print(f"8/21 收盘口径总资产: {total_mv:,.2f}")
...
print("\n参考: 8/21 盘后重估总资产 391,364.54（周报口径）")
...
out = {"date": "2026-08-24", "as_of": "2026-08-21收盘", "snap_total": round(snap_total, 2),
       ...
       "detail": detail}
with open(os.path.join(BASE, "data/processed/history/portfolio_preopen_20260824.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260824.json")

# -*- coding: utf-8 -*-
"""盘前档 2026-09-14：组合估值重算（9/11 收盘口径）+ 赛道占比 + 集中度 + 夏普
净值：A股类场外 9/11（周一早盘全量可抓）、QDII 9/10（T+1）
场内 ETF：9/11 收盘20:00；个股：9/11 真实收盘（广联达 8.23 / 通威 11.05）
"""
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-11" and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
# ---------- 个股 9/11 真实收盘 ----------
STOCK_PRICE = {"002410": 8.23, "600438": 11.05}
...
print(f"\n9/11 收盘口径总资产: {total_mv:,.2f}")
print(f"（9/11 盘后归档基准 373,320.35）差额 {total_mv - 373320.35:,.2f}")
...
detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
          ...
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": "2026-09-14", "as_of": "2026-09-11收盘(净值A股类9/11/QDII 9/10)",
...
json.dump(out, open(os.path.join(HIST, "portfolio_preopen_20260914.json"), "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260914.json")

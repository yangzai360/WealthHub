# -*- coding: utf-8 -*-
"""盘中档 2026-09-22：且慢长赢(LONG_WIN) playwright 兜底数据落库（§3.82：mtime 最新 + 校验 composition）
   - plan 详情 → composition-2026-09-22.json + meta.json（含 nav_date_beijing）
...
本档 REST pmdj adjustments 仍空 body（连续第 88 日）→ playwright 兜底
"""
...
if os.path.exists(prev_path):
    ...
    print(f'对照快照 composition-{PREV}.json 品种数={len(prev_pu)}  planUnit 合计={sum(v for v in prev_pu.values() if v)}')
else:
    print(f'⚠️ 无 {PREV} 对照快照')

# -*- coding: utf-8 -*-
"""盘前档 2026-09-24(周四) 行情抓取:
1) 场外基金净值增量 —— 天天基金 F10 直连(§3.79); 判重 key = (code, nav_date) 二元组(§3.86)
   ★本档重点: 9/23 盘后档对 8 只被动联接/债券类用「指数代理」估算(§3.98)
     → 本档须取 9/23 真实净值做兜底修正, 并生成 portfolio_close_20260923_fix.json
   ★QDII 三只(000369/016280/164906): 复核 9/23 净值是否出库(对应美股 9/23 收盘, 预期 T+1 尚未出)
2) 美股(美东 9/23 周三收盘 = 隔夜, ★新增交易日) —— XLV/IYH/QQQ/DIA/SPY + .IXIC/.DJI/.INX
   ★XLV 9/23 收盘将用于回填 9/23 留空的 11 条「美股标普医药」事件
3) A股指数日线(9/23 收盘复核) + 个股/ETF 复核 + 港股 hq 直连
4) 且慢 pmdj 复测 + 代理探针
...
# 9/23 盘后档用指数代理估算的 8 只（本档须取 9/23 真实净值兜底）
PROXY_WATCH = {'000968', '002742', '004752', '001180', '000051', '000071', '001469', '012323'}
...
for code, nm in fund_map.items():
    ...
    tag = ''
    if code in PROXY_WATCH:
        tag += ' ***PROXY-FIX(9/23兜底)***'

# -*- coding: utf-8 -*-
"""2026-09-11 盘后: 链路累计/W37周累计/夏普比率/最大回撤（date 缺失时从文件名推导 §3.55）"""
...
# 链路起点 8/6（首次执行, -0.68%）无 portfolio_close 产物 → 显式补入（与历史报告口径一致）
...
w = 1.0
wk = [(d, p) for d, p, _, _ in rows if d >= '2026-09-07']
for d, p in wk:
    w *= (1 + p / 100)
print(f'本周 W37（9/7 起 {len(wk)} 日）累计: {(w-1)*100:.2f}%  ' + ' / '.join(f'{d[5:]} {p:+.2f}%' for d, p in wk))

rets = [p for _, p, _, _ in rows]
n = len(rets)
mean = st.mean(rets)
sd = st.stdev(rets) if n > 1 else 0
rf_daily = 1.69 / 252
...
out = {'date': '2026-09-11', 'chain_cum_pct': round((cum-1)*100, 2), 'chain_days': len(rows),
       'w37_cum_pct': round((w-1)*100, 2), 'sharpe': round(sharpe, 2) if sharpe else None,
       'sharpe_n': n, 'daily_mean': round(mean, 4), 'daily_sd': round(sd, 4),
       'max_dd_pct': round(mdd*100, 2)}
json.dump(out, open(os.path.join(HIST, 'risk_stats_20260911.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('\n已保存 risk_stats_20260911.json')

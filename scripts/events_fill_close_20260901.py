# -*- coding: utf-8 -*-
"""盘后档 2026-09-01：①回填 9/1 当日 26 条
②追加盘后 7 条事件(id 027-033)并回填 ③全库完整性扫描
...
# ---------- 9/1 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': 0.53,          # ETF均值（医疗ETF +0.59% / 医药ETF广发 +0.47%）；医药生物 +0.48%、创新药午后走强
    '大消费': 1.15,           # 中证消费 12,623.92 +1.15% 收稳12,600（白酒概念 +2.35%、商贸零售 +3.08%、农林牧渔 +2.48%）
    '恒生科技': -1.49,        # HSTECH 4,550.88 -1.49% 收日内最低、守4,500（恒指 -0.93% 25,329.73）
    '宏观': -0.16,            # 上证指数 3,979.89 -0.16% 距4,000仅20点（创业板 -1.32%、深成指 -1.02%、成交20,334亿缩量）
    '其他/宽基': 0.55,        # 广联达 -0.89% / 通威 +1.98% 平均（传媒ETF +1.19%、证券ETF +0.72%）
    # 美股标普医药 8/30+8/31 遗留已由 9/1 盘前用 XLV 8/31 -0.36% 回填完成；9/1 当日无美股医药新事件
}
def get_ret1d(e):
...
print(f'9/1 当日回填 {fix} 条；留空 {len(left)} 条')
for x in left:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

# ---------- 2. 追加盘后 7 条事件（id 027-033）+ 回填 ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-09-01.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev)

# 历史样本统计（供 reference，不含 9/1）
all_events = []
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-09-01.json'), encoding='utf-8'))
for n in news_all:
...
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-09-01.json 共 {len(combined)} 条')
for e in new_events:

# -*- coding: utf-8 -*-
"""盘中档 2026-09-18：且慢长赢(LONG_WIN) playwright 兜底数据落库（§3.82 规则：mtime 最新 + 校验 composition）
   - plan 详情 → composition-2026-09-18.json + meta.json（含 nav_date_beijing）
   - nav-history 增量：经核对**不追加**（源序列近期数值与本地已落库行不再 1:1 匹配，见输出诊断）
...
has_new = old_cnt != new_cnt
print(f'本地 meta.adjustedCount={old_cnt} vs 源={new_cnt} -> '
      f'{"无新调仓" if not has_new else "⚠️ 有新调仓，需人工复核"}')
...
print(f'  -> 本地最新 {max(loc)}；源最新(北京) {max(src)}；**本档不追加任何行**（待用户决策口径修正）')

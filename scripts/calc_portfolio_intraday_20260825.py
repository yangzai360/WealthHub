# -*- coding: utf-8 -*-
"""2026-08-25 盘中: 组合当日收益估算（8/24 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/25 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:42-13:45 抓取）
intraday = {
    # 场内ETF
    "513050": 0.09, "159928": 0.61, "512170": 2.06, "513180": -0.34,
    "159920": -0.13, "512880": -0.09, "512980": 1.51, "515180": -0.76,
    "159938": 1.40,
    # 指数
    "sh000001": 0.13, "sz399006": -0.85, "sh000932": 0.42,
    "HSTECH": -0.43, "HSI": -0.16,
    # 美股(8/24 XLV +0.05%/IYH -0.07%, QDII 8/26-27 才兑现, 今日盘中按 0)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/24 收盘口径市值 (portfolio_preopen_20260825.json detail)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260825.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 +1.73
...
print(f"总资产(8/24收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/24收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260825.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260825.json")

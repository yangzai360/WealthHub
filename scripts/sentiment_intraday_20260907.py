# -*- coding: utf-8 -*-
"""2026-09-07 盘中: DeepSeek 情绪标注 8 条盘中新增新闻 (分2批, 每批≤7; 标题截断≤200字符规避空响应)"""
...
with open("/tmp/intraday_news_20260907.json", encoding="utf-8") as f:
    news = json.load(f)
...
{json.dumps([{ "i": i+1, "title": trunc_title(n["title"]), "summary": n["summary"]} for i, n in enumerate(batch)], ensure_ascii=False, indent=2)}
...
sent_path = os.path.join(BASE, "data/processed/news/sentiment-2026-09-07.json")
...
print(f"\nsentiment-2026-09-07.json 共 {len(sent)} 条 (盘前{len(sent)-added} + 盘中{added})")

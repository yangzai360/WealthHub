# -*- coding: utf-8 -*-
"""2026-08-26 盘中: 组合当日收益估算（8/25 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/26 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:44-13:45 抓取）
intraday = {
    # 场内ETF
    "513050": 1.21, "159928": 0.30, "512170": -0.86, "513180": 1.04,
    "159920": 0.73, "512880": 2.25, "512980": -0.62, "515180": 0.48,
    "159938": 0.00,
    # 指数
    "sh000001": 0.59, "sz399006": 0.83, "sh000932": 0.06,
    "HSTECH": 1.24, "HSI": 0.54,
    # 美股(8/24 XLV +0.05%/IYH -0.07%, QDII 8/26-27 兑现 -0.11%, 今日盘中按 0)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/25 收盘口径市值 (portfolio_preopen_20260826.json detail)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260826.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 -0.43
...
def approx_pct(code, name):
    ...
    if code6 in ("000369", "016280"):
        return intraday["IYH"]          # 美股医药QDII -> IYH(8/24, 今日按0)
    if code6 in ("000071",):
...
print(f"总资产(8/25收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/25收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260826.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260826.json")

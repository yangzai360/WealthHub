# -*- coding: utf-8 -*-
"""盘前档 2026-08-26：①事件库追加 16 条（从 news/sentiment 读取，actual_ret_1d 待 8/26 收盘回填）
②全库完整性扫描 ③1日样本统计+方向验证 ④3日窗口统计"""
...
SENT_PATH = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-26.json')
...
for i, s in enumerate(sentiments, 1):
    ...
    evs.append({
        ...
        "reference": {
            ...
            "confidence": None,
            "actual_ret_1d": None,
            "note": "盘前事件,actual_ret_1d 待 8/26(周三)收盘回填",
...
# ---------- 3. 1 日样本统计 + 方向验证（不含今日 16 条，未回填不计入） ----------
...
# ---------- 4. 3 日窗口统计（8/6-8/20 事件，事件日之后 3 个交易日累计） ----------
print('\n--- 3 日窗口统计 ---')
idx_series = defaultdict(dict)
for fname in ['indices.csv']:
    with open(os.path.join(BASE, 'data/processed/history', fname), encoding='utf-8-sig') as f:
...
for d, x in ev_dated:
    ...
    # 3 日窗口：事件日之后连续 3 个交易日（需 idx_pos+3 <= len-1）
    ...
    # 5 日窗口：事件日之后连续 5 个交易日（需 idx_pos+5 <= len-1）

# -*- coding: utf-8 -*-
"""盘后档 2026-08-31：①回填 8/31 当日 25 条（美股标普医药留空待 9/1）
②追加盘后 8 条事件(id 026-033)并回填 ③回填 8/30 周末 15 条（美股医药留空待 9/1）
④全库完整性扫描 ⑤1日样本统计+方向验证 ⑥3/5日窗口统计更新"""
...
# ---------- 8/31 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': -1.13,         # ETF均值（医疗ETF -0.88% / 医药ETF广发 -1.38%）；医药生物 -1.35%、创新药 -1.65%
    '大消费': -0.72,           # 中证消费 12,480.96 -0.72% 再失12,500（白酒8月市值前五全跌、茅台 -0.43%）
    '恒生科技': 0.32,          # HSTECH 4,619.87 +0.32% 收复4,600（盘中最低4,531.22、午后转涨；恒指 -0.07%）
    '宏观': 0.86,              # 上证指数 3,986.30 +0.86% 距4,000仅14点（创业板 +0.42%、科创50 +1.34%、成交2.13万亿）
    '其他/宽基': -1.60,        # 广联达 -0.11% / 通威 -3.09% 平均（传媒ETF +3.83% 为亮点但宽基整体弱）
    # 美股标普医药留空（8/31 美股未收盘，待 9/1 用 XLV/IYH 8/31 收盘回填）
}
def get_ret1d(e):
...
# ---------- 1. 回填 8/31 当日 25 条 ----------
ev_path = os.path.join(EV, 'events-2026-08-31.json')
ev31 = json.load(open(ev_path, encoding='utf-8'))
fix31 = 0
for x in ev31:
    ...
    if t in track_ret:
        ...
        fix31 += 1
json.dump(ev31, open(ev_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
left31 = [x for x in ev31 if get_ret1d(x) is None]
print(f'8/31 当日回填 {fix31} 条；留空 {len(left31)} 条（美股标普医药待 9/1）')
for x in left31:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

# ---------- 2. 追加盘后 8 条事件（id 026-033）+ 回填 ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-31.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev31)

# 历史样本统计（供 reference，不含 8/31）
all_events = []
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-31.json'), encoding='utf-8'))
for n in news_all:
...
combined = ev31 + new_events
json.dump(combined, open(ev_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-31.json 共 {len(combined)} 条')
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']}/{e['strength']} ret1d={e['reference']['actual_ret_1d']} | {e['title'][:40]}")

# ---------- 3. 回填 8/30 周末 15 条（用 8/31 收盘；美股标普医药留空待 9/1） ----------
ev30_path = os.path.join(EV, 'events-2026-08-30.json')
ev30 = json.load(open(ev30_path, encoding='utf-8'))
fix30 = 0
for x in ev30:
    if get_ret1d(x) is not None:
        continue
    t = x.get('track')
    if t in track_ret:
        set_ret1d(x, track_ret[t], '首个交易日(8/31)收盘')
        fix30 += 1
json.dump(ev30, open(ev30_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
left30 = [x for x in ev30 if get_ret1d(x) is None]
print(f'\n8/30 周末回填 {fix30} 条；留空 {len(left30)} 条（美股标普医药待 9/1）')
for x in left30:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

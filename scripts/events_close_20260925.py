# -*- coding: utf-8 -*-
"""2026-09-25 盘后档事件库处理：
  ① 追加本档 23 条盘后事件（window == '盘后(14:00-20:00)'，含空集硬守卫）
  ② 全库完整性扫描（留空按 日期×赛道 二维统计，§3.98）
  ③ 历史相似事件匹配（品类 + 赛道 + 关键词 Jaccard → Top3，统计 3/5/10 日涨跌与波动）
  ④ 事件库 3/5/10 日窗口统计 → history/event_stats_close_20260925.json
⚠️ 本档 A股休市（中秋），9/25 全部事件参考交易日 = 9/28（复市首日），一律留空待回填（预期状态，非缺陷）
⚠️ 口径：A股医药 3/5/10 日窗口序列以 399006（创业板指）为代理（§3.90），报告须披露。
⚠️ §3.106/§3.107：写盘路径一律由 TODAY 变量推导
"""
...
for t, arr in sorted(agg.items()):
    ...
    print(f"  {t:10s} n={len(arr):>2d}  3日={sum(a3)/len(a3) if a3 else float('nan'):+.2f}%  "
...
out = {'date': TODAY, 'window': 'close', 'total_events': len(all_ev), 'today_events': len(ev),
       ...
       'similarity_by_track': {t: {'n': len(arr),
                                   ...
                                   'n_win_na': {'3': sum(1 for x in arr if x['avg_3d'] is None),
                                                '5': sum(1 for x in arr if x['avg_5d'] is None),
                                                '10': sum(1 for x in arr if x['avg_10d'] is None)}}
       ...
       'note': '本档 A股休市（中秋），9/25 全部事件参考交易日 = 9/28（复市首日），一律留空属预期状态；A股医药 3/5/10 日窗口以 399006 创业板指为代理（§3.90）'}
json.dump(out, open(os.path.join(HIST, f'event_stats_close_{TODAY.replace("-","")}.json'), 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1)
print(f'\n已保存 event_stats_close_{TODAY.replace("-","")}.json')

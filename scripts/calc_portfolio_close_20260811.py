...
with open(os.path.join(HIST, "fund_nav.csv"), encoding="utf-8-sig") as f:
    for r in csv.DictReader(f):
        if r["date"] == "2026-08-11" and r["nav_date"] == "2026-08-11":
            try:
                FUND_REAL[r["code"]] = float(r["pct"])
            except Exception:
                pass
print(f"\n8/11 净值已更新基金数: {len(FUND_REAL)} 只")
for c, v in FUND_REAL.items():
    print(f"  {c}: {v}%")

# ---------- 个股收盘 ----------
STOCK_CLOSE = {}
with open(os.path.join(HIST, "close_20260811.json"), encoding="utf-8") as f:
    cj = json.load(f)
...
med_etf = ETF_CLOSE.get("159938", -0.15)  # 医药ETF广发
TRACK_PROXY = {
    "A股医药": med_etf,                       # 医药ETF广发收盘（冲高回落）
    "大消费": IDX_CLOSE.get("000932", -1.14),  # 中证消费
    "美股标普医药": 0.0,                        # QDII 净值滞后(T+1~T+2, 广发全球医疗停在 8/7), 8/10 XLV+1.67% 明日反映
    "恒生科技": IDX_CLOSE.get("HSTECH", -1.93),
    "其他/宽基": IDX_CLOSE.get("000001", -0.82),
    "现金": 0.0,
}
def est_pct(row):
...
with open(os.path.join(HIST, "portfolio_close_20260811.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),
...
print("\n已保存 portfolio_close_20260811.json")

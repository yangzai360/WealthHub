# -*- coding: utf-8 -*-
"""2026-08-31 盘中: 事件库追加 8 条盘中新闻 (id 018-024) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-31.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-31.json')
...
TRACK_MAP = {
    "中国8月官方制造业PMI 49.8%": "宏观",
    "A股午评（8/31）": "宏观",
    "港股午评（8/31）": "恒生科技",
    "A股医药续跌（8/31盘中）": "A股医药",
    "港股创新药ETF重挫4%+（8/31）": "A股医药",
    "白酒分化定价首日温和（8/31盘中）": "大消费",
    "传媒AIGC爆发（8/31盘中）": "其他/宽基",
    "沃什鹰派+美元走强压制贵金属（8/31）": "宏观",
}
def get_track(title):
...
print(f"events-2026-08-31.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

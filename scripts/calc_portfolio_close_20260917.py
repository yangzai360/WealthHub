# -*- coding: utf-8 -*-
"""2026-09-17 盘后：组合收盘估算（链式法，§3.79/§3.85/§3.88 口径）
base = portfolio_preopen_20260917.json 的 detail（= 9/16 收盘值：场内 9/16 收盘、场外 A股类 9/16 净值、QDII/LOF 9/15 净值）
BASE_TOTAL = 375,953.14（9/16 盘后归档链式值 = 当日涨跌基准）
day_pct 逐券 = 最新可得价 / base 隐含价 - 1
  · 场内 = 9/17 收盘价
  · 场外 A股类 = 9/17 净值（19 只已出）
  · QDII/LOF（000369/016280/164906）= 9/16 净值（本档新可得，T+1 路径自然生效）
  · 泓德裕祥债券A（002742）9/17 净值未出（最新 9/16 = base）→ 按 0 并标注
"""
import json, os
...
TODAY = '2026-09-17'
BASE_TOTAL = 375953.14
QDII = ('000369', '016280', '164906')

base = json.load(open(os.path.join(HIST, 'portfolio_preopen_20260917.json'), encoding='utf-8'))
...
out = {'date': TODAY, 'as_of': '2026-09-17收盘', 'base_total': BASE_TOTAL,

# -*- coding: utf-8 -*-
"""盘前档 2026-09-16: 新建 events-2026-09-16.json + 历史事件库匹配统计
回填检查：9/15 当日 8 条美股标普医药 actual_ret_1d 待 9/15 美股收盘回填（XLV -0.05%）
"""
import json, os, glob
...
NEWS_FILE = os.path.join(BASE, 'data/processed/news/news-2026-09-16.json')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-16.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-16.json')
DATE = '2026-09-16'

# ---------- 0. 前一日(9/15)回填完整性检查（兼容 reference 嵌套, §3.73） ----------
prev = os.path.join(EVENTS_DIR, 'events-2026-09-15.json')
...
print(f'9/15 当日事件 {len(prev_evs)} 条, 留空 {len(prev_null)} 条')

# ---------- 0b. 盘前档兜底回填：9/15 留空的美股标普医药 -> XLV 9/15 收盘 -0.05% ----------
XLV_0915 = -0.05
if prev_null:
    filled = 0
    for e in prev_evs:
        ...
        if ref.get('actual_ret_1d') is None:
            ref['actual_ret_1d'] = XLV_0915
            ref['actual_date'] = '2026-09-15'
            ref['ret_1d_ref'] = '9/16 盘前档兜底回填（美股 9/15 收盘口径 XLV -0.05%，§3.70）'
            filled += 1
    json.dump(prev_evs, open(prev, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    print(f'  -> 已兜底回填 {filled} 条（XLV 9/15 {XLV_0915}%）')
    left = []
...
assert all(e['id'].startswith('N20260916-') for e in new_events), 'id 前缀错误'

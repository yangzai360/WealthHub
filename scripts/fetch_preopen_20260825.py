# -*- coding: utf-8 -*-
"""盘前档 2026-08-25(周二) 行情抓取：场外基金净值增量(8/24周一净值, 周二早盘全量可抓, QDII 8/21 待兑现) + 隔夜美股 8/24(美东周一)收盘归档"""
...
# ---------- 1. 场外基金净值增量（8/24 周一净值，周二早盘全量可抓，QDII T+1~T+2 至 8/21） ----------
fund_map = {
...
# ---------- 3. 验证 A股/港股 8/24 收盘已在库 ----------
with open(idx_path, encoding='utf-8-sig') as f:
    for row in csv.DictReader(f):
        if row['date'].startswith('2026-08-24') and '收盘' in row.get('note', ''):
            print(f"  [已归档] {row['name']} {row['date']} close={row['close']} pct={row.get('pct', '')}%")

# ---------- 4. 且慢 pmdj 复测（第 25 日, 预期空 body 标注即可） ----------
try:

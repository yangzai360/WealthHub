# -*- coding: utf-8 -*-
"""2026-09-16 盘中档：新闻/情绪/事件库增量落库（§3.82 口径：标题统一 title[:60] 判重）"""
import json, os, glob
...
news = json.load(open('/tmp/news_intraday_20260916.json'))


def cat_of(n):
    t = n['title']
    if any(k in t for k in ['央行', '逆回购', '国新办', '统计局', '经济数据', '社零', 'FOMC', '油价',
                            '布伦特', 'Shibor', '宁德时代', '浙商证券', '华泰证券', '建行']):
        return '宏观类'
    if any(k in t for k in ['规划', '印发', '政策', '获批', '批准', '受理', '国家药监局', '医保']):
        return '政策类'
    if any(k in t for k in ['半年', '业绩', '净利润', '营收', '销售额', '销售量', '中报']):
        return '业绩类'
    return '行业事件类'
...
for n in news:
    ...
    added.append({
        ...
        "summary": n['brief'], "source": "财联社/东方财富/证券时报/新华财经/中新经纬/央广网/新浪财经/腾讯新闻",
...
# ---- 2) sentiment 文件增量（title 截断 60 字符） ----
...
for n in news:
    ...
    new_ev.append({
        ...
        "source": "财联社/东方财富/证券时报/新华财经/中新经纬/央广网/新浪财经/腾讯新闻",

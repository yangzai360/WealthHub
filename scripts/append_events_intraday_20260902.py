# -*- coding: utf-8 -*-
"""2026-09-02 盘中: 事件库追加 8 条盘中新闻 (id 018-024) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-02.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-02.json')
...
TRACK_MAP = {
    "A股午评（9/2）": "宏观",
    "港股午评（9/2）": "恒生科技",
    "A股医药创新药逆势活跃（9/2盘中）": "A股医药",
    "医保局发布按病种付费3.0版分组方案和技术规范（9/2）": "A股医药",
    "中证消费回落12,600下方（9/2盘中）": "大消费",
    "传媒AIGC高位分歧（9/2盘中）": "其他/宽基",
    "港股黄金股重挫+金价续跌（9/2盘中）": "宏观",
    "恒生科技13:45实时4,487（-1.40%）跌破4,500防线": "恒生科技",
}
def get_track(title):
...
print(f"events-2026-09-02.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']} | {e['title'][:40]}")

print("\n=== 历史事件 1日收益统计（按赛道） ===")
for t, s in track_stats.items():
    print(f"  {t}: n={s['n']} avg={s['avg_ret_1d']} worst={s['worst_ret_1d']} pos={s['pos_ratio']} ret3d={s['avg_ret_3d']} ret5d={s['avg_ret_5d']} ret10d={s['avg_ret_10d']}")

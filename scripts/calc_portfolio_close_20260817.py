# -*- coding: utf-8 -*-
"""盘后档 2026-08-17：组合真实收盘收益重算 + 赛道归因"""
...
# ---------- 8/17 真实收盘涨跌幅 ----------
# 场内 ETF 收盘（close_20260817.json）
ETF_CLOSE = {
    "159938": 0.74, "512170": 0.57, "159928": -0.89, "513180": 1.68,
    "513050": 1.83, "159920": 1.49, "512880": 0.55, "512980": -0.93,
    "515180": 0.21,
}
# 场外基金 8/17 当日净值（已出）
FUND_NAV_817 = {
    "519915": 0.53,   # 富国消费主题 8/17 +0.53%（已出）
}
# QDII 8/14 补更（T+1 未出 8/17，用 8/14 美股医疗走势近似，标注 T+1）
FUND_NAV_814_UPDATE = {
    "000369": -0.60,   # 广发全球医疗A 8/14 美股 XLV -0.60%（T+1 兑现）
    "016280": -0.60,   # 广发全球医疗C 同上
    "164906": -1.75,   # 交银海外互联 8/13 补更已归档；8/17 中概互联ETF +1.83% 用 ETF 代理
    "000071": 1.49,    # 华夏恒生ETF联接 用 159920 恒生ETF华夏 +1.49% 代理
    "012348": 1.58,    # 天弘恒生科技 用 HSTECH +1.58% 代理
}
# A股场外基金（T+1 未出 8/17，用对应指数/ETF 代理）
FUND_PROXY = {
    "002708": 0.66,   # 大摩健康 用 医药ETF均值 +0.66%
    "000727": 0.66,   # 融通健康 同上
    "161616": 0.66,   # 融通医疗 同上
    "001180": 0.66,   # 广发医药卫生 同上
    "012323": 0.66,   # 华宝中证医疗 同上
    "001551": 0.66,   # 天弘医药100 同上
    "000248": -1.14,  # 汇添富主要消费 用 中证消费 -1.14% 代理
    "004424": -0.93,  # 汇添富文体娱乐 用 传媒ETF -0.93% 代理
    "000968": 0.50,   # 广发养老 用 消费医药均衡近似
    "005368": 0.86,   # 富国清洁能源 用 通威 +0.86% 代理（光伏）
    "110020": 1.41,   # 易方达沪深300 用 上证指数 +1.41% 代理
    "000051": 1.41,   # 华夏沪深300 同上
    "001469": 0.55,   # 广发金融地产 用 证券ETF +0.55% 代理
    "001552": 0.55,   # 天弘证券保险 同上
    "100032": 0.21,   # 富国红利增强 用 100红利 +0.21% 代理
    "004752": -0.93,  # 广发传媒 用 传媒ETF -0.93% 代理
    "002742": 0.0,    # 泓德裕祥债券 0
}
# 个股真实收盘
STOCK_CLOSE = {"002410": -0.55, "600438": 0.86}
def est_pct(row):
    ...
    if code6 in FUND_NAV_817:
        return FUND_NAV_817[code6]
    if code6 in FUND_NAV_814_UPDATE:
        return FUND_NAV_814_UPDATE[code6]
...
with open(os.path.join(BASE, "data/processed/history/portfolio_close_20260817.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),
...
print("\n已保存 portfolio_close_20260817.json")

# -*- coding: utf-8 -*-
"""2026-09-03 盘中: 事件库追加 8 条盘中新闻 (id 020-027) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-03.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-03.json')
...
TRACK_MAP = {
    "A股午评（9/3）": "宏观",
    "港股午评（9/3）": "恒生科技",
    "恒生科技13:45实时4,472.33（-0.99%）再度跌破4,500防线": "恒生科技",
    "A股医药盘中走弱但相对抗跌（9/3）": "A股医药",
    "中证消费失守12,500（9/3盘中）": "大消费",
    "央行连续两日7天期逆回购零投放": "宏观",
    "大金融护盘+贵金属避险走强（9/3盘中）": "宏观",
    "传媒短剧AIGC情绪延续第3日（9/3盘中）": "其他/宽基",
}
def get_track(title):
...
print(f"events-2026-09-03.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

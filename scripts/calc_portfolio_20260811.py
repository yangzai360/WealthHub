# -*- coding: utf-8 -*-
"""持仓合并 + 赛道占比 + 盘中组合估算收益 (2026-08-11 13:45 档)"""
...
PROXY = {
    "A股医药": 0.80,        # 医药ETF +1.02 / 医疗ETF +0.57 均值
    "大消费": -1.05,         # 消费ETF -1.16 / 中证消费 -0.95 均值
    "美股标普医药": 0.0,     # QDII T+1, 8/10 IYH 已反映(近似 0)
    "恒生科技": -0.87,       # HSTECH -1.35 / 恒指科技ETF -0.65 / 中概互联 -0.60 均值
    "其他/宽基": -0.21,      # 上证 -0.21 (宽基近似)
    "现金": 0.0,
}
ETF_REALTIME = {
    "159938": 1.02, "512170": 0.57, "159928": -1.16, "513180": -0.65,
    "513050": -0.60, "159920": -0.26, "512880": -0.09, "512980": -0.68,
    "515180": -0.35,
}
# 个股盘中价暂缺, 用指数近似: 广联达(软件)→创业板指 +1.30, 通威(光伏)→上证 -0.21 保守
STOCK_PROXY = {"002410": 1.30, "600438": -0.21}
def est_pct(row):
...
with open(os.path.join(BASE, "data/processed/history/portfolio_20260811.json"), "w", encoding="utf-8") as f:
    json.dump({
        "total": round(total, 2),

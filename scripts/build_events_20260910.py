# -*- coding: utf-8 -*-
"""盘前档 2026-09-10: ①回填 N20260909-002(美股标普医药, XLV 9/9 -0.33%) ②新建 events-2026-09-10.json (16 条) + 历史事件库匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
NEWS_FILE = os.path.join(BASE, 'data/processed/news/news-2026-09-10.json')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-10.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-10.json')

# ---------- 0. 回填 9/9 遗留美股标普医药 N20260909-002 (XLV 9/9 -0.33%) ----------
ev9_path = os.path.join(EVENTS_DIR, 'events-2026-09-09.json')
ev9 = json.load(open(ev9_path, encoding='utf-8'))
filled = 0
for e in ev9:
    if e['id'] == 'N20260909-002':
        r = e.setdefault('reference', {})
        r['actual_ret_1d'] = -0.33
        r['actual_date'] = '2026-09-09'
        r['ret_1d_ref'] = 'XLV 9/9 -0.33%(Lp(a)利空第2日企稳)'
        filled += 1
json.dump(ev9, open(ev9_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'回填 N20260909-002: XLV 9/9 -0.33% -> {filled} 条')
...
print(f"events-2026-09-10.json 新建 {len(new_events)} 条")
for e in new_events:
    print(f"  {e['id']} [{e['track']}] {e['sentiment']}{e['score']}/{e['strength']} | {e['title'][:38]}")

print("\n=== 历史事件 1日收益统计（按赛道, 截至 9/9 收盘回填） ===")
...
print(f"\n全库事件 {total} 条, 留空 {nulls} 条(9/10 当日 16 条待收盘回填属正常)")

# -*- coding: utf-8 -*-
"""盘前档 2026-09-07：组合估值重算（9/4 收盘真实净值全量口径，QDII 广发全球医疗 9/3 +0.23%/+0.23% 已兑现、交银海外互联 9/3 -1.26%）+ 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 9/3、其余 9/4）；场内 ETF/个股用 9/4 收盘价"""
...
print("fund_nav 最新净值（QDII 9/3 兑现、其余 9/4）：")
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-04" and "收盘" in row.get("note", ""):
...
# ---------- 个股 9/4 收盘 ----------
STOCK_PRICE = {"002410": 8.86, "600438": 11.39}   # 广联达 8.86(+3.02%)、通威 11.39(+0.98%)
...
print(f"\n9/4 收盘口径总资产(净值全量修正): {total_mv:,.2f}")
...
print("\n参考: 9/4 盘后代理口径 387,357.57；本次为场外 9/4 净值全量 + QDII 9/3 兑现修正口径")
...
out = {"date": "2026-09-07", "as_of": "2026-09-04收盘(净值全量修正,QDII 9/3兑现)", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260907.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260907.json")

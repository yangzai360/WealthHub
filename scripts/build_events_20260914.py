# -*- coding: utf-8 -*-
"""盘前档 2026-09-14: 新建 events-2026-09-14.json (27 条) + 历史事件库匹配统计
...
NEWS_FILE = os.path.join(BASE, 'data/processed/news/news-2026-09-14.json')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-14.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-14.json')
DATE = '2026-09-14'

# ---------- 0. 前一日(9/13)回填完整性检查（兼容 reference 嵌套, §3.73） ----------
prev = os.path.join(EVENTS_DIR, 'events-2026-09-13.json')
...
print(f'9/13 周末事件 {len(prev_evs)} 条, 留空 {len(prev_null)} 条')
print(f'  -> 9/13 为周末事件, actual_ret_1d 以 9/14(首个交易日)收盘回填, 属预期留空（§3.22 口径）, 由 9/14 盘后档执行')
...
assert all(e['id'].startswith('N20260914-') for e in new_events), 'id 前缀错误'

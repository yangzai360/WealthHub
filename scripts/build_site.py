...
def mask_amounts(text: str, protected: bool = False) -> str:
    ...
    if protected:
        ...
        # ② 裸的六位数千分位金额（组合总资产/基准量级 = 100,000~999,999）
        #    §隐私坑: 报告常写「还原后合计 = 383,039.00」「基准由 383,064.62 → ***」等不带「元」的裸金额。
        #    判别依据: 本组合所有指数点位最大为恒生指数 24,761（首位 1~2 位数字），
        #    故 `\d{3},\d{3}`（首位恰为 3 位数）只可能命中金额,不会误伤指数点位。
        text = re.sub(r"\d{3},\d{3}(?:\.\d+)?", "***", text)
        return text

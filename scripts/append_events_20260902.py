# -*- coding: utf-8 -*-
"""盘前档 2026-09-02：①事件库追加 18 条（actual_ret_1d 待 9/2 收盘回填）②回填 9/1 遗留美股标普医药 2 条（用 XLV 9/1 +0.66%）③全库完整性扫描 ④1日样本+方向验证 ⑤3/5日窗口统计"""
...
SENT_PATH = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-02.json')
...
for i, s in enumerate(sentiments, 1):
    ...
    evs.append({
        ...
        "reference": {
            ...
            "confidence": None,
            "actual_ret_1d": None,
            "note": "盘前事件,actual_ret_1d 待 9/2(周三)收盘回填",
...
# ---------- 2. 回填 9/1 遗留美股标普医药事件（XLV 9/1 +0.66%） ----------
FILL_IDS = ['N20260901-012', 'N20260901-013']
FILL_VAL = 0.66
FILL_NOTE = '美股 9/1 收盘回填(XLV +0.66%, 新浪源)'
filled = 0
for f in sorted(glob.glob(os.path.join(EV, 'events-2026-09-0*.json'))):

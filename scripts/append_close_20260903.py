# -*- coding: utf-8 -*-
"""盘后档 2026-09-03 增量写入：indices(A股3收盘+港股2) / etf_intraday(收盘9) / fund_nav(9/3当日已出净值)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260903.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股收盘（hq 直取 9/3：恒指 25213.31 -0.39% / 恒科 4468.48 -1.08%，与新华社通稿一致）
for h in data['hk']:
...
# ---------- 3. fund_nav (code, nav_date) 判重：9/3 当日已出净值 ----------
...
for n in data['fund_navs']:
    ...
    # 只写 nav_date == TODAY 的当日净值（9/3 已出）；QDII T+1 补更另用放宽过滤（§3.47）
...
if new_fund:
    with open(fund_path, 'a', encoding='utf-8') as f:
        for row in new_fund:
            f.write(','.join(row) + '\n')
    print(f'\nfund_nav 新增 {len(new_fund)} 行（9/3 当日已出净值）:')
    for r in new_fund:
        print('  ' + ','.join(r))
else:
    print('fund_nav 无新增（9/3 当日无新净值）')

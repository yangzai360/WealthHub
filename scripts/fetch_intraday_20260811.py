# -*- coding: utf-8 -*-
"""
盘中行情抓取 (2026-08-11 13:45 档)
源: akshare 新浪源 + 天天基金源 (东财 push2 已被代理拦截, 不适用)
输出: data/processed/history/{indices,etf_intraday,fund_nav}.csv (增量) + intraday_20260811.json
"""
...
def append_unique(path, rows, key_prefix_len, header):
    """增量追加: 用前 key_prefix_len 列(拼接)判重, 同 key 保留旧行(盘中不覆盖收盘/盘前)"""
...
], 4, "type,date,name,code,close,pct_change,note")
...
], 3, "date,code,name,price,pct,amount_wan,note")
...
], 4, "date,code,name,nav_date,nav,pct")
print(f"[fund_nav] 新增 {n_nav} 条")

# 汇总 JSON
with open(os.path.join(HIST, "intraday_20260811.json"), "w", encoding="utf-8") as f:

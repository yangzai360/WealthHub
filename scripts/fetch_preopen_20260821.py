# -*- coding: utf-8 -*-
"""盘前档 2026-08-21(周五) 行情抓取：场外基金净值增量(8/20周四净值) + 隔夜美股(8/20收盘) + A股/恒科指数参考 + 且慢pmdj复测(第18日)"""
...
# ---------- 1. 场外基金净值增量（8/20 周四净值，周五早盘全量可抓，QDII T+1~T+2） ----------
fund_map = {
...
# ---------- 4. 港股指数 spot（盘前第9日预期失败，直接用已归档 8/20 收盘） ----------
hk_done = {}
...
for attempt in range(2):
    ...
    time.sleep(2)
if len(hk_done) < 2:
    print('  WARN 港股 spot 盘前仍空（连续第9日），沿用 indices.csv 8/20 收盘')

# ---------- 5. 且慢 pmdj 复测（第18日，预期空 body） ----------
qieman = {}
try:
...
with open(os.path.join(HIST, 'preopen_20260821.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> preopen_20260821.json')

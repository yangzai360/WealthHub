# -*- coding: utf-8 -*-
"""盘后档 2026-09-02 收盘行情抓取：A股指数日线 / 港股spot(重试,失败走新浪hq直取) / 场内ETF / 个股 / 场外净值 / 且慢pmdj复测"""
...
# ---------- 2. 港股指数 spot（重试）→ 失败走新浪 hq 直取（§3.57 优先级：新浪 hq > 新闻口径） ----------
                ...
                for code, cname in hk_map.items():
                    if code in code_raw or (code == 'HSI' and '恒生指数' in nm) or (code == 'HSTECH' and '恒生科技' in nm):
                        if code not in hk_done:
                            ...
                            print(f'  {cname}(spot): {row["最新价"]} pct={row["涨跌幅"]}%')
            if len(hk_done) == 2:
                break
    except Exception as e:
...
# spot 失败 → 新浪 hq 直取（带 Referer，GBK）
if len(hk_done) < 2:
    try:
        ...
        for line in raw.strip().split('\n'):
            if 'hkHSI' in line:
                ...
                # 恒生指数: 名称,今开,昨收,最高,最低,最新,涨跌额,涨跌幅%,...
                if len(parts) > 8 and parts[7]:
                    hk_done['HSI'] = {'type': 'index', 'date': TODAY, 'name': '恒生指数', 'code': 'HSI',
                                      'close': round(float(parts[6]), 2), 'pct_change': round(float(parts[7]), 2),
                                      'note': '收盘20:00(新浪hq直取)'}
                    print(f"  恒生指数(hq): {parts[6]} pct={parts[7]}%")
            elif 'hkHSTECH' in line:
                parts = line.split('"')[1].split(',')
                if len(parts) > 8 and parts[7]:
                    hk_done['HSTECH'] = {'type': 'index', 'date': TODAY, 'name': '恒生科技', 'code': 'HSTECH',
                                         'close': round(float(parts[6]), 2), 'pct_change': round(float(parts[7]), 2),
                                         'note': '收盘20:00(新浪hq直取)'}
                    print(f"  恒生科技(hq): {parts[6]} pct={parts[7]}%")
    except Exception as e:
        print(f'  WARN 新浪 hq 港股直取失败: {e}')
for code in hk_map:
    if code not in hk_done:
...
with open(os.path.join(HIST, 'close_20260902.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> close_20260902.json')

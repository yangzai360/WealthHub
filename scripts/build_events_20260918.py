# -*- coding: utf-8 -*-
"""盘前档 2026-09-18: ①兜底回填 9/17 遗留 8 条美股标普医药留空（XLV 9/17 收盘 +0.62%）
②新建 events-2026-09-18.json（28 条）+ 历史事件库匹配统计
"""
import json, os, glob
...
NEWS_FILE = os.path.join(BASE, 'data/processed/news/news-2026-09-18.json')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-18.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-09-18.json')
DATE = '2026-09-18'

# ---------- 0. 前一日(9/17)兜底回填：8 条美股标普医药 -> XLV 9/17 收盘 +0.62% ----------
prev = os.path.join(EVENTS_DIR, 'events-2026-09-17.json')
...
print(f'9/17 当日事件 {len(prev_evs)} 条, 留空 {len(prev_null)} 条')
...
if prev_null:
    ...
    for e in prev_evs:
        ...
        if ref.get('actual_ret_1d') is None:
            ref['actual_ret_1d'] = XLV_0917
            ref['actual_date'] = '2026-09-17'
            ref['ret_1d_ref'] = '9/18 盘前档兜底回填（美股 9/17 收盘口径 XLV +0.62%，§3.70）'
            filled += 1
    json.dump(prev_evs, open(prev, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    print(f'  -> 已兜底回填 {filled} 条（XLV 9/17 +{XLV_0917}%）')
...
assert all(e['id'].startswith('N20260918-') for e in new_events), 'id 前缀错误'

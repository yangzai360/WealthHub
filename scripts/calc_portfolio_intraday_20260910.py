# -*- coding: utf-8 -*-
"""2026-09-10 盘中: 组合当日收益估算（9/9 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 9/10 盘中涨跌（%）：场内ETF实时(13:45 akshare) + 指数实时(13:45 akshare, hq 直连交叉验证)
intraday = {
    # 场内ETF (13:45 实时 akshare)
    "513050": -1.94, "159928": -1.91, "512170": -0.89, "513180": -1.79,
    "159920": -1.08, "512880": 0.37, "512980": -1.54, "515180": -0.75,
    "159938": -1.10,
    # 指数 (13:45 akshare, hq 直连交叉验证: 上证3931.56 -0.50/创业板3336.51 -0.55)
    "sh000001": -0.49, "sz399006": -0.49, "sh000932": -1.72, "sh000300": -0.56,
    "HSTECH": -2.09, "HSI": -1.29,
    # 个股实时 (sina hq 13:45)
    "002410": -2.11,   # 广联达 8.37 (高位题材退潮跟跌, 距 9.0 更远)
    "600438": -1.62,   # 通威 11.52
    # 美股医药 QDII: XLV 9/9 -0.33% 于 9/10-11 兑现 → 盘中估算按 0
    "XLV": 0.0, "IYH": 0.0,
    # 光伏代理(清洁能源基金参考) 通威-1.62% → 光伏ETF代理
    "515790": -1.62,
}

# 持仓基准: 9/9 收盘口径市值 (portfolio_preopen_20260910.json)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260910.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 -1.00%
...
def approx_pct(code, name):
    ...
    if code6 in ("002708", "161616", "001180", "001551", "012323", "000727"):
        return MED_AVG                  # A股医药 -> 医疗/医药ETF均值 -1.00%
    if code6 in ("000248", "519915", "000968"):
        return intraday["sh000932"]     # 大消费 -> 中证消费 -1.72%
    if code6 in ("004424",):
        return intraday["512980"]       # 文体娱乐 -> 传媒ETF -1.54%
    if code6 in ("012348",):
        return intraday["513180"]       # 恒生科技QDII -> 恒指科技ETF -1.79%
    if code6 in ("164906",):
        return intraday["513050"]       # 中概互联 -> 中概互联ETF -1.94%
    if code6 in ("000369", "016280"):
        return intraday["IYH"]          # 美股医药QDII -> IYH(今日兑现按0)
    if code6 in ("000071",):
        return intraday["159920"]       # 恒生ETF联接 -> 恒生ETF华夏 -1.08%
    if code6 in ("000051", "110020"):
        return intraday["sh000300"]     # 沪深300 -> 沪深300指数 -0.56%
    if code6 in ("001469", "001552"):
        return intraday["512880"]       # 金融地产/证券保险 -> 证券ETF +0.37%
    if code6 in ("005368",):
        return intraday["515790"]       # 清洁能源 -> 光伏代理(通威) -1.62%
    if code6 in ("100032",):
        return intraday["515180"]       # 红利 -> 100红利 -0.75%
    if code6 in ("004752",):
...
print(f"总资产(9/9收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(9/9收盘口径) ==")
...
print("\n  主要贡献标的:")
for name, p, w in sorted(items, key=lambda x: -abs(x[2]))[:12]:
...
with open("/tmp/portfolio_intraday_20260910.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260910.json")

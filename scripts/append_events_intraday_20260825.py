# -*- coding: utf-8 -*-
"""2026-08-25 盘中: 事件库追加 8 条盘中新闻 (id 016-023) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-25.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-25.json')
...
TRACK_MAP = {
    "A股午评（8/25）": "宏观",
    "财政部/央行/金融监管总局联合印发": "大消费",
    "创新药概念回暖（8/25）": "A股医药",
    "港股午评（8/25）": "恒生科技",
    "医药板块早盘表现活跃多股涨停": "A股医药",
    "贵州茅台8/25盘中维持1,300上方": "大消费",
    "国际贵金属冲高回落（8/25）": "宏观",
    "阿里800亿港元配售获近3倍超额认购": "恒生科技",
}
def get_track(title):
...
print(f"events-2026-08-25.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

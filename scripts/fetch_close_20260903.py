# -*- coding: utf-8 -*-
"""盘后档 2026-09-03 收盘行情抓取：A股指数日线 / 港股spot(重试,失败走新浪hq) / 场内ETF / 个股 / 场外净值 / 且慢pmdj复测"""
...
# spot 失败 → 新浪 hq 直取（带 Referer，GBK；注意 parts[8] 为涨跌幅%）
if len(hk_done) < 2:
    try:
            ...
            if 'hkHSI' in line:
                ...
                if len(parts) > 8 and parts[8]:
                    hk_done['HSI'] = {'type': 'index', 'date': TODAY, 'name': '恒生指数', 'code': 'HSI',
                                      'close': round(float(parts[6]), 2), 'pct_change': round(float(parts[8]), 2),
                                      'note': '收盘20:00(新浪hq直取)'}
                    print(f"  恒生指数(hq): {parts[6]} pct={parts[8]}%")
                ...
                if len(parts) > 8 and parts[8]:
                    hk_done['HSTECH'] = {'type': 'index', 'date': TODAY, 'name': '恒生科技', 'code': 'HSTECH',
                                         'close': round(float(parts[6]), 2), 'pct_change': round(float(parts[8]), 2),
                                         'note': '收盘20:00(新浪hq直取)'}
                    print(f"  恒生科技(hq): {parts[6]} pct={parts[8]}%")
    except Exception as e:
...
with open(os.path.join(HIST, 'close_20260903.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> close_20260903.json')

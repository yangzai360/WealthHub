# -*- coding: utf-8 -*-
"""盘前档 2026-09-17：组合估值重算（9/16 收盘口径）+ 赛道占比 + 集中度 + 夏普
净值：A股类场外 9/16 已落地（9/16 盘后全量）；QDII/LOF 最新 9/15（000369/016280/164906）
场内 ETF：9/16 收盘；个股：9/16 真实收盘（广联达 8.40 / 通威 11.21 / 茅台 1,258.00）
基准：9/16 盘后归档链式值 375,953.14
"""
...
TODAY = "2026-09-17"
BASE_TOTAL = 375953.14          # 9/16 盘后归档链式值
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-16" and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
print(f"9/16 收盘 ETF {len(ETF_SPOT)} 只: {ETF_SPOT}")
# ---------- 个股 9/16 真实收盘 ----------
STOCK_PRICE = {"002410": 8.40, "600438": 11.21}
...
print(f"\n9/16 收盘口径总资产（独立重算）: {total_mv:,.2f}")
print(f"（9/16 盘后归档链式基准 {BASE_TOTAL:,.2f}）差额 {total_mv - BASE_TOTAL:,.2f} "
...
detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
          ...
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": TODAY, "as_of": "9/16收盘(场外A股类9/16全量/QDII·LOF 9/15)",

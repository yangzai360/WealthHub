# -*- coding: utf-8 -*-
"""盘后档 2026-08-28：①回填 8/28 当日 24 条（美股标普医药留空待 8/30）
...
# ---------- 8/28 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': -1.04,         # ETF均值（医疗ETF -1.16% / 医药ETF广发 -0.91%）；创新药概念 -1.09%、CRO走弱（凯莱英 -3.33%）
    '大消费': 0.66,           # 中证消费 12,571.99 +0.66% 站上12,500（老窖除息日 +1.20% 软着陆第3次验证、白酒 +0.29% 普涨胜出）
    '恒生科技': -0.33,        # HSTECH 4,605.15 -0.33% 守4,600（恒指 25,584.79 +0.07% 期指结算日倒升）
    '宏观': -0.11,            # 上证指数 3,952.18 -0.11%（创业板 -1.41%、科创50 -1.85% 冲高回落、成交2.1万亿缩量242亿）
    '其他/宽基': 1.45,        # 广联达 +2.39% / 通威 +0.50% 平均
    # 美股标普医药留空（8/28 美股未收盘，待 8/30 用 XLV/IYH 回填）
}
def get_ret1d(e):
...
# ---------- 1. 回填 8/28 当日 24 条 ----------
ev_path = os.path.join(EV, 'events-2026-08-28.json')
ev28 = json.load(open(ev_path, encoding='utf-8'))
fix28 = 0
for x in ev28:
    ...
    if t in track_ret:
        ...
        fix28 += 1
json.dump(ev28, open(ev_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
left28 = [x for x in ev28 if get_ret1d(x) is None]
print(f'8/28 当日回填 {fix28} 条；留空 {len(left28)} 条（美股标普医药待 8/30）')
for x in left28:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

# ---------- 2. 追加盘后 8 条事件（id 025-032）+ 回填 ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-28.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev28)

# 历史样本统计（供 reference，不含 8/28）
all_events = []
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-28.json'), encoding='utf-8'))
for n in news_all:
...
combined = ev28 + new_events
json.dump(combined, open(ev_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-28.json 共 {len(combined)} 条')
for e in new_events:

# -*- coding: utf-8 -*-
"""盘前档 2026-08-20(周四) 行情抓取：场外基金净值增量(8/19周三净值) + 隔夜美股(8/19收盘) + A股/恒科指数参考 + 且慢pmdj复测(第16日)"""
...
# ---------- 1. 场外基金净值增量（8/19 周三净值，周四早盘全量可抓，QDII 停 8/18 T+1） ----------
fund_map = {
...
# ---------- 4. 港股指数 spot（盘前第8日预期失败，直接用已归档 8/19 收盘） ----------
hk_done = {}
...
for attempt in range(2):
    ...
    time.sleep(2)
if len(hk_done) < 2:
    print('  WARN 港股 spot 盘前仍空（连续第8日），沿用 indices.csv 8/19 收盘')

# ---------- 5. 且慢 pmdj 复测（第16日，预期空 body） ----------
qieman = {}
try:
...
with open(os.path.join(HIST, 'preopen_20260820.json'), 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print('\nDONE -> preopen_20260820.json')

# -*- coding: utf-8 -*-
"""2026-08-28 盘中: 事件库追加 7 条盘中新闻 (id 018-024) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-28.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-28.json')
...
# 盘中 7 条手动归类赛道（track 用赛道名，禁止 category 名）
TRACK_MAP = {
    "A股午评（8/28）": "宏观",
    "港股午评（8/28）": "恒生科技",
    "A股医药全线回调（8/28盘中）": "A股医药",
    "港股通创新药ETF连涨3日后回落（8/28）": "A股医药",
    "泸州老窖除权除息日不跌反涨（8/28）": "大消费",
    "沃什杰克逊霍尔前夜（8/28）": "宏观",
    "韩国央行连续第二次加息25bp至3%（8/28）": "宏观",
}
def get_track(title):
...
print(f"events-2026-08-28.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

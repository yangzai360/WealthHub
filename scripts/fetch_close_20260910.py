# -*- coding: utf-8 -*-
"""2026-09-10 盘后档行情抓取: A股指数日线收盘/港股hq直取/场内ETF收盘/场外基金当日净值+QDII补更/个股真实收盘
输出 data/processed/history/close_20260910.json"""
...
# ---------- 2. 港股收盘（hq.sinajs.cn 直取, 20:00 与新华社通稿等价 §3.62/§3.74需Referer） ----------
def fetch_hq():
...
# ---------- 5. 个股真实收盘 (A股 新浪源) ----------
stocks = [('sz002410', '广联达'), ('sh600438', '通威股份')]
for sym, name in stocks:
    ...
    if df is not None and len(df) >= 2:
        ...
        print(f'  个股 {name} 真实收盘 {round(cur_c,2)} {pct}%')
    else:

# -*- coding: utf-8 -*-
"""2026-08-31 盘中: 组合当日收益估算（8/28 收盘市值为基准 + 盘中实时涨跌）+ 赛道敞口"""
import json, os

base = "/Users/jieyang/Documents/WealthHub"

# 8/31 盘中涨跌（%）：场内ETF实时 + 指数盘中（13:44 抓取）
intraday = {
    # 场内ETF
    "513050": -0.74, "159928": -0.60, "512170": -1.17, "513180": -0.86,
    "159920": -0.67, "512880": -0.63, "512980": 3.83, "515180": 0.55,
    "159938": -1.69,
    # 指数
    "sh000001": 0.23, "sz399006": -0.56, "sh000932": -0.58,
    "HSTECH": -0.37, "HSI": -0.45,
    # 美股(8/28 XLV -0.24% 于 9/1-2 兑现, 盘中估算按 0)
    "XLV": 0.0, "IYH": 0.0,
}

# 持仓基准: 8/28 收盘口径市值 (portfolio_preopen_20260831.json)
with open(os.path.join(base, "data/processed/history/portfolio_preopen_20260831.json"), encoding="utf-8") as f:
    preopen = json.load(f)
holdings = [(d["account"], d["name"], d["code"], d["mv"], d["track"]) for d in preopen["detail"]]

MED_AVG = round((intraday["512170"] + intraday["159938"]) / 2, 2)  # A股医药场外代理均值 -1.43
...
print(f"总资产(8/28收盘口径): {total:,.2f} 元")
print("\n== 赛道敞口(8/28收盘口径) ==")
...
with open("/tmp/portfolio_intraday_20260831.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)
print("\n已存 /tmp/portfolio_intraday_20260831.json")

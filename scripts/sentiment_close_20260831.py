# -*- coding: utf-8 -*-
"""盘后档 2026-08-31：DeepSeek 情绪标注盘后新增 8 条 + 合并全量 + 同步回 news"""
import json, os, urllib.request, time, re

BASE = '/Users/jieyang/Documents/WealthHub'
NEWS = os.path.join(BASE, 'data/processed/news/news-2026-08-31.json')
SENT = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-31.json')
...
todo = [n for n in news if n['title'] not in sent_by_title]
...
for n in todo:
    ...
    item = {'date': n['date'], 'track': n.get('track', '宏观'), 'title': n['title'],
            'sentiment': r.get('sentiment'), 'score': r.get('score'),
            'strength': r.get('strength', r.get('score', 50)),

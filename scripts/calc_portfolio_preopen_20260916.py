# -*- coding: utf-8 -*-
"""盘前档 2026-09-16：组合估值重算（9/15 收盘口径）+ 赛道占比 + 集中度 + 夏普
净值：A股类场外 9/15 全量（9/15 盘后已落地）、QDII/LOF 最新 9/14（000369/016280/164906）
场内 ETF：9/15 收盘；个股：9/15 真实收盘（广联达 8.37 / 通威 11.15 / 茅台 1,272.75）
"""
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-15" and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
# ---------- 个股 9/15 真实收盘 ----------
STOCK_PRICE = {"002410": 8.37, "600438": 11.15}
...
print(f"\n9/15 收盘口径总资产: {total_mv:,.2f}")
print(f"（9/15 盘后归档基准 375,505.37）差额 {total_mv - 375505.37:,.2f}")
...
detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
          ...
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": "2026-09-16", "as_of": "9/15收盘(场外A股类9/15全量/QDII·LOF 9/14)",
...
json.dump(out, open(os.path.join(HIST, "portfolio_preopen_20260916.json"), "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260916.json")

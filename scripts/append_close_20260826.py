# -*- coding: utf-8 -*-
"""盘后档 2026-08-26 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(8/26当日净值)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260826.json'), encoding='utf-8') as f:
    data = json.load(f)
...
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25652.97', '0.56', '收盘20:00(新闻口径补录)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4626.15', '0.82', '收盘20:00(新闻口径补录)'],
]
for h in hk_news:
    key = (h[1], h[3], h[6])
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= '2026-08-26':

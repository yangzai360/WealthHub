# -*- coding: utf-8 -*-
"""2026-09-07 盘后: close_20260907.json -> indices.csv/etf_intraday.csv/fund_nav.csv 增量写入
...
# 3. fund_nav（只写 nav_date>=2026-09-05，避免历史行重复）
nav_rows = [['2026-09-07', r['code'], r['name'], r['nav_date'], r['nav'], r['pct'] if r['pct'] is not None else '']
            for r in close['fund_navs'] if r['nav_date'] >= '2026-09-05']
n3 = append_unique(os.path.join(HIST, 'fund_nav.csv'), nav_rows, [1, 3],
                   'date,code,name,nav_date,nav,pct')
print(f'[fund_nav] 新增 {n3} 条 (nav_date>=9/5)')

# 校验: 当日 indices 含 A股 5 + HSI/HSTECH 收盘行 (§3.59 教训)
with open(os.path.join(HIST, 'indices.csv'), encoding='utf-8-sig') as f:
    lines = f.readlines()
today_rows = [l.strip() for l in lines if '2026-09-07' in l and '收盘20:00' in l]

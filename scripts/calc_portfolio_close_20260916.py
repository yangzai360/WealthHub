# -*- coding: utf-8 -*-
"""2026-09-16 盘后：组合收盘估算（链式法，§3.79/§3.85 口径）
base = portfolio_preopen_20260916.json 的 detail（= 9/15 收盘值：场内 9/15 收盘、场外 A股类 9/15 净值、QDII/LOF 9/14 净值）
BASE_TOTAL = 375,505.37（9/15 盘后归档链式值 = 当日涨跌基准）
day_pct 逐券 = 最新可得价 / base 隐含价 - 1
  · 场内 = 9/16 收盘价
  · 场外 A股类 = 9/16 净值（19 只已出）
  · QDII/LOF（000369/016280/164906）= 9/15 净值（本档新可得，T+1 路径自然生效）
  · 000051 华夏沪深300联接 9/16 净值未出（最新 9/15 = base）→ 按 0 并标注
"""
import json, os
...
TODAY = '2026-09-16'
BASE_TOTAL = 375505.37

base = json.load(open(os.path.join(HIST, 'portfolio_preopen_20260916.json'), encoding='utf-8'))
...
for x in base['detail']:
    ...
    if code6 == '' or sh == 0:
        ...
        if nd == '2026-09-16':
            src = f'净值 9/16 ({nav})'
        elif code6 in ('000369', '016280', '164906'):
...
out = {'date': TODAY, 'as_of': '2026-09-16收盘', 'base_total': BASE_TOTAL,
...
print('\n个券贡献（按 |pnl| 排序）:')
for d in sorted(detail, key=lambda y: -abs(y['est_pnl']))[:22]:

# -*- coding: utf-8 -*-
"""盘前档 2026-08-24(周一) 行情抓取：场外基金净值增量(8/21周五净值, 周一早盘全量可抓, QDII T+1~T+2) + 美股/A股验证(8/21已在库, 仅打印参考)"""
...
# ---------- 1. 场外基金净值增量（8/21 周五净值，周一早盘全量可抓，QDII T+1~T+2） ----------
fund_map = {
...
# ---------- 2. 验证美股/A股最新归档日期（8/21 已在库，仅打印） ----------
with open(os.path.join(HIST, 'indices.csv'), encoding='utf-8-sig') as f:
    for row in csv.DictReader(f):
        if row['code'] in ('XLV', 'IYH', 'QQQ', '.IXIC') and '美股收盘' in row['note']:
            print(f"  [已归档] {row['name']} {row['date']} close={row['close']} pct={row['pct']}%")

# ---------- 3. 且慢 pmdj 复测（第 22 日, 预期空 body 标注即可） ----------
try:

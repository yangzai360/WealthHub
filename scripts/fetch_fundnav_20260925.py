# -*- coding: utf-8 -*-
"""2026-09-25 盘后：场外基金净值检查（天天基金 F10 直连）
重点：①QDII 000369/016280 是否出 9/24 净值（对应美股 9/24 收盘 XLV+0.63%/IYH+0.75%）
      ②164906 是否出 9/24 净值 ③012348 天弘恒生科技A 是否出 9/25 净值（港股开市）
      ④A股类基金是否出 9/25 净值（预期：无，A股休市）
"""
...
print('\n净值日分布(>=9/20):', dict(Counter(n['nav_date'] for n in navs if n['nav_date'] >= '2026-09-20')))
print('\n=== 9/24 与 9/25 净值明细 ===')
for n in navs:
    if n['nav_date'] >= '2026-09-24':
        print(f"  {n['nav_date']}  {n['code']}  {n['name']:22s} {n['nav']:>8.4f}  {n['pct']:>+7.2f}%")
print('\n抓取失败:', fail)

# -*- coding: utf-8 -*-
"""2026-09-09 盘后: DeepSeek 情绪标注盘后新增 8 条 (幂等, 2x4 分批 §3.67)
只标注 sentiment 为空的条目; 结果写回 sentiment-2026-09-09.json"""
import json, os, time, urllib.request

BASE = '/Users/jieyang/Documents/WealthHub'
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-09.json')

with open(os.path.join(BASE, 'data/processed/news/news-2026-09-09.json'), encoding='utf-8') as f:
    news = json.load(f)
...
# 待标注: news 中不在 sent 或 sentiment 为空的
pending = []
for n in news:
    t = n.get('title', '')
    if t in sent_titles:
...
if not pending:
    ...
    results = {}
    # 2x4 分批 (每批 ≤4, 每批最多4次重试 §3.67)
    # 注意: DeepSeek 返回的 i 是批内 1-based, 需 + bi 转全局索引, 否则第二批覆盖第一批 (§3.62 索引坑变体)
    for bi in range(0, len(pending), 4):
        ...
        for attempt in range(4):
            try:
                ...
                time.sleep(5)
        if not ok:
            print(f'  批次{bi//4+1} 重试耗尽，跳过')
        else:
            print(f'  批次{bi//4+1} ({len(batch)}条) 标注成功')
...
# 输出标注结果概览
for s in sent:
    print(f"  {s.get('sentiment','?'):2s} {s.get('score','?'):>3s}/{(str(s.get('strength',''))):>3s} | {(s.get('title') or '')[:45]}")

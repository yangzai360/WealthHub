#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-12 盘前档行情增量更新
范围: 隔夜美股 8/11 收盘(XLV/IYH/QQQ/DIA) + 场外基金 8/11 净值增量
...
if us_rows:
    append_rows(INDICES, us_rows)
    print(f"[US] 写入 {len(us_rows)} 行 indices.csv")

# ---------- 2. 场外基金净值增量 (8/11 收盘或最新) ----------
funds = [
    # (code, name)
    ...
    try:
        ...
        if pct in (None, "", "-", "--"):
            if len(df) >= 2:
                ...
                pct = 0.0
        # 增量: nav_date >= 2026-08-10 (8/10 已归档, 8/11 净值新出)
        if nav_date >= "2026-08-10":
            row = f"{today},{code},{name},{nav_date},{nav},{pct}"
            nav_rows.append(row)
            print(f"[NAV] {code} {name} {nav_date} nav={nav} pct={pct}% 候选")
        else:

# -*- coding: utf-8 -*-
"""盘前档 2026-08-20：组合估值重算（8/19 收盘真实净值/收盘价）+ 赛道占比 + 集中度/回撤检查"""
...
# ---------- 8/19 收盘真实净值/收盘价（今日盘前抓取全量） ----------
# 场外基金 8/19 单位净值（今日盘前抓取；QDII 标注 8/18 为 T+1~T+2 滞后）
FUND_NAV_819 = {
    "002708": 2.01, "000968": 0.8388, "002742": 1.261, "004752": 0.8105,
    "005368": 1.1413, "110020": 1.8772, "100032": 0.972, "001180": 0.8329,
    "161616": 1.851, "000051": 1.752, "519915": 1.869, "000071": 1.5069,
    "012348": 0.6403, "001551": 0.7569, "000248": 1.8287,
    "001469": 1.222, "001552": 1.0218, "012323": 0.5931,
    "000727": 2.513, "004424": 1.7954,
    "000369": 2.617, "016280": 2.574, "164906": 0.961,   # QDII 8/18（T+1 滞后，8/19 美股 +3.52% 待 8/21-22 兑现）
}
# 场内 ETF 8/19 收盘价（close_20260819.json 实测）
ETF_PRICE_819 = {
    "513050": 1.114, "159928": 0.664, "512170": 0.344, "513180": 0.59,
    "515180": 1.427, "159920": 1.498, "512880": 1.062, "512980": 0.803,
    "159938": 0.669,
}
# 个股 8/19 收盘
STOCK_PRICE_819 = {"002410": 8.87, "600438": 12.30}
...
def calc_market_value(row):
    ...
    if code6 in ETF_PRICE_819:
        return shares * ETF_PRICE_819[code6], row["amount"]
    if code6 in STOCK_PRICE_819:
        return shares * STOCK_PRICE_819[code6], row["amount"]
    if code6 in FUND_NAV_819:
        return shares * FUND_NAV_819[code6], row["amount"]
...
print(f"8/19 收盘口径总资产: {total_mv:,.2f}")
...
# 8/19 组合当日估算（相对 8/18 收盘 393,861.28）
print("\n参考: 8/18 收盘总资产 393,861.28；8/19 估算增量 =", round(total_mv - 393861.28, 2),
      f"({(total_mv-393861.28)/393861.28*100:.2f}%)")
...
with open(os.path.join(BASE, "data/processed/history/portfolio_preopen_20260820.json"), "w", encoding="utf-8") as f:
    json.dump({"date": "2026-08-20", "snap_total": round(snap_total, 2), "total_mv": round(total_mv, 2),
...
print("\n已保存 portfolio_preopen_20260820.json")

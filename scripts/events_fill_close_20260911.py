# -*- coding: utf-8 -*-
"""盘后档 2026-09-11：①追加盘后 9 条事件 ②回填 9/11 全部事件 actual_ret_1d
...
# ---------- 9/11 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': -1.78,    # ETF均值（医疗ETF -1.80% / 医药ETF广发 -1.75%）；申万医药生物 -1.94%
    '大消费': -1.01,     # 中证消费 12,349.57 -1.01%（收盘确认失守 12,400）
    '恒生科技': -0.23,   # HSTECH 4,320.57 -0.23%（4,300 防线守住、午后一度翻红）
    '宏观': -1.18,       # 上证指数 3,888.11 -1.18%（放量 1.97 万亿、4,870 股下跌、主力净流出 410.49 亿）
    '其他/宽基': -2.91,  # 个股均值（广联达 -1.91% / 通威 -3.91%）；证券ETF -3.02% / 传媒ETF -1.32%
    # 美股标普医药: 9/11 美股未收盘（北京 9/12 凌晨），事件待 9/12 用 XLV 回填
}
...
# ---------- 1. 追加盘后 9 条事件（从 sentiment 差集, 补 track） ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-09-11.json'), encoding='utf-8'))
...
TRACK_MAP = {
    ...
    "第七批国家组织高值医用耗材集采": "A股医药", "恒瑞医药（600276": "A股医药",
    "美国FCC相关最终规则": "其他/宽基", "DeepSeek 正式发布": "其他/宽基",
    "穆巴达拉": "大消费",
}
...
print(f'盘后新增事件 {len(new_events)} 条 -> 9/11 共 {len(ev)} 条')
for e in new_events:
...
# ---------- 2. 回填 9/11 全部事件（美股标普医药留空待 9/12 XLV） ----------
fix = 0
for x in ev:
    ...
    t = x.get('track')
    if t == '美股标普医药':
        continue  # 美股 9/11 未收盘，9/12 盘前用 XLV 回填
    if t in track_ret:
        set_ret1d(x, track_ret[t], '9/11赛道收盘(ETF均值/指数/个股均值)')
        fix += 1
json.dump(ev, open(ev_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'9/11 回填 {fix} 条 (美股标普医药留空待 9/12 XLV)')

# -*- coding: utf-8 -*-
"""盘前档 2026-09-15：组合估值重算（9/14 收盘口径，含 2 只 T+1 净值补更修正）+ 赛道占比 + 集中度 + 夏普
净值：A股类场外 9/14 全量（含 012323/000071 由 T+1 代理修正为真实值）、QDII 9/11
场内 ETF：9/14 收盘；个股：9/14 真实收盘（广联达 8.33 / 通威 11.27）
"""
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-14" and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
# ---------- 个股 9/14 真实收盘 ----------
STOCK_PRICE = {"002410": 8.33, "600438": 11.27}
...
print(f"\n9/14 收盘口径总资产: {total_mv:,.2f}")
print(f"（9/14 盘后归档基准 375,841.62）差额 {total_mv - 375841.62:,.2f}")
...
detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
          ...
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": "2026-09-15", "as_of": "9/14收盘(场外净值A股类9/14全量/QDII 9/11)",
...
json.dump(out, open(os.path.join(HIST, "portfolio_preopen_20260915.json"), "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260915.json")

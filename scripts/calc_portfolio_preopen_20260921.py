# -*- coding: utf-8 -*-
"""盘前档 2026-09-21：组合估值重算（9/18 收盘口径）+ 赛道占比 + 集中度 + 夏普
净值：A股类场外 9/18 已全量落地；QDII/LOF 最新 9/17（000369/016280/164906，9/18 净值未出库）
场内 ETF：9/18 收盘；个股：9/18 真实收盘（广联达 8.40 / 通威 11.35）
基准：9/18 盘后归档链式值（净值补更修正后）378,405.26
"""
...
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == PREV_DATE and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
print(f"9/18 收盘 ETF {len(ETF_SPOT)} 只: {ETF_SPOT}")

# ---------- 个股 9/18 真实收盘 ----------
STOCK_PRICE = {"002410": 8.40, "600438": 11.35}
...
def calc_market_value(row):
    ...
    if "余额宝" in name or "货币" in name or "现金" in name:
        return row["amount"], 0.0, "现金"
    if code6 in ETF_SPOT:
        return shares * ETF_SPOT[code6], row["amount"], f"场内收盘 {ETF_SPOT[code6]}"
    if code6 in STOCK_PRICE:
        return shares * STOCK_PRICE[code6], row["amount"], f"个股收盘 {STOCK_PRICE[code6]}"
    if code6 in fund_nav:
        return shares * fund_nav[code6][1], row["amount"], f"净值 {fund_nav[code6][0]} ({fund_nav[code6][1]})"
...
print(f"\n9/18 收盘口径总资产（独立重算）: {total_mv:,.2f}")
print(f"（9/18 盘后归档链式基准 {BASE_TOTAL:,.2f}）差额 {total_mv - BASE_TOTAL:,.2f} "
...
for f in sorted(glob.glob(os.path.join(HIST, "portfolio_close_2026*.json"))) + \
    ...
    if pct is None:
        continue
    # ⚠️ §3.55/§3.91：日期一律从「文件名」推导（JSON date 字段会漏样本）
...
if len(daily_rets) >= 10:
    ...
    # §3.91：单位必须统一（daily_rets 为百分数 → rf 亦须为百分数日频）
    rf_d = rf_10y / 252
...
print(f"链路累计（{daily_rets[0][0]} 起 {len(daily_rets)} 日）: {chain:+.2f}%  最大回撤 {mdd:.2f}%")

detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
           "mv": round(r["mv"], 2), "shares": r["shares"], "price_src": r["price_src"]}
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": TODAY, "as_of": "9/18收盘(场外A股类9/18全量/QDII·LOF 9/17)",

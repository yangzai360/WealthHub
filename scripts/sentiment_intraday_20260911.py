# -*- coding: utf-8 -*-
"""2026-09-11 盘中: DeepSeek 情绪标注 9 条盘中新增新闻 (3批×4/4/1; 标题截断≤200字符规避空响应; 批次全局索引偏移防覆盖)"""
...
with open("/tmp/intraday_news_20260911.json", encoding="utf-8") as f:
    news = json.load(f)

def trunc_title(t, n=200):
...
{json.dumps([{ "i": i+1, "title": trunc_title(n["title"]), "summary": n["summary"][:400]} for i, n in enumerate(batch)], ensure_ascii=False, indent=2)}
...
sent_path = os.path.join(BASE, "data/processed/news/sentiment-2026-09-11.json")
...
print(f"\nsentiment-2026-09-11.json 共 {len(sent)} 条 (盘前{len(sent)-added} + 盘中{added})")

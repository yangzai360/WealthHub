# -*- coding: utf-8 -*-
"""2026-08-21 盘中: 事件库追加 6 条盘中新闻 (id 014-019) + 历史匹配统计"""
import json, os, glob

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-21.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-21.json')
...
# 盘中 6 条手动归类赛道（track 用赛道名，禁止 category 名）
TRACK_MAP = {
    "A股午评：沪指微跌0.11%": "宏观",
    "医药主升第2日大分歧回调": "A股医药",
    "港股午评：恒指+0.72%": "恒生科技",
    "财政部10:00发布会落地": "宏观",
    "中证消费盘中跌破12,500防线": "大消费",
    "8/21白酒批价": "大消费",
}
def get_track(title):
...
print(f"events-2026-08-21.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

# -*- coding: utf-8 -*-
"""盘后档 2026-08-25 增量写入：indices(A股3收盘+港股2新闻口径) / etf_intraday(收盘9) / fund_nav(8/25当日净值)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260825.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股新闻口径补录（spot 连续第10日盘后失败，本次用新华社/新浪财经/每经收评多源交叉：恒指 25511.10 -0.02% / 恒科 4588.54 -0.12%）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25511.10', '-0.02', '收盘20:00(新闻口径补录)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4588.54', '-0.12', '收盘20:00(新闻口径补录)'],
]
for h in hk_news:
    key = (h[1], h[3], h[6])
...
# ---------- 3. fund_nav (code, nav_date) 判重，8/25 当日净值 ----------
...
for n in data['fund_navs']:
    ...
    if key not in existing_fund and n['nav'] is not None and n['nav_date'] >= '2026-08-25':
...
if new_fund:
    ...
    print('fund_nav 无 8/25 当日新增（T+1 未更新，正常）')

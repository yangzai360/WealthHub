# -*- coding: utf-8 -*-
"""2026-08-24 盘中: 事件库追加 7 条盘中新闻 (id 015-021) + 历史匹配统计"""
import json, os, glob

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-24.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-24.json')
...
TRACK_MAP = {
    "A股午评：三大指数集体走弱": "宏观",
    "港股午评：恒指-2.09%": "恒生科技",
    "医药板块深度回调：中证创新药指数-4.2%": "A股医药",
    "2026版国家基本药物目录将于9月1日实施": "A股医药",
    "贵州茅台逆市涨超3%重回1300元": "大消费",
    "英伟达财报周三（8/27凌晨）": "宏观",
    "拼多多今晚（8/24美股盘前）发布Q2财报": "恒生科技",
}
def get_track(title):
...
print(f"events-2026-08-24.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

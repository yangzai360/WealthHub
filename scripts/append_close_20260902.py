# -*- coding: utf-8 -*-
"""盘后档 2026-09-02 增量写入：indices(A股3收盘+港股2新浪hq) / etf_intraday(收盘9) / fund_nav(9/2当日已出)"""
import json, os, csv
...
with open(os.path.join(HIST, 'close_20260902.json'), encoding='utf-8') as f:
    data = json.load(f)
...
# 港股收盘补录（spot 盘后连续第16日失败；新浪 hq 直取 9/2 收盘：恒指 25275.27 -0.21% / 恒科 4513.26 -0.83% 收复4,500）
hk_news = [
    ['index', TODAY, '恒生指数', 'HSI', '25275.27', '-0.21', '收盘20:00(新浪hq直取)'],
    ['index', TODAY, '恒生科技', 'HSTECH', '4513.26', '-0.83', '收盘20:00(新浪hq直取)'],
]
if new_idx:
...
# ---------- 3. fund_nav (code, nav_date) 判重：9/2 当日已出净值 ----------
...
if new_fund:
    with open(fund_path, 'a', encoding='utf-8') as f:
        for row in new_fund:
            f.write(','.join(row) + '\n')
    print(f'\nfund_nav 新增 {len(new_fund)} 行（9/2 当日已出净值）:')
    for r in new_fund:
        print('  ' + ','.join(r))
else:
    print('fund_nav 无新增（9/2 当日无新净值）')

# -*- coding: utf-8 -*-
"""盘前档 2026-09-23：① 修正 9/22 收盘（161616/000727 两只主动医药基真实净值兜底，§3.95 _fix 模式）
                  ② 生成 9/23 盘前基准（= 修正后 9/22 收盘链式值）+ 赛道占比 + 风险指标

背景（9/22 盘后遗留）：161616 融通医疗保健、000727 融通健康产业 的 9/22 净值 20:00 时点未出库，
盘后档按「板块代理 × 全样本中位弹性（§3.99 D2 否定档）」估算（-0.1281% / -0.0963%）。
本档已抓到真实 9/22 净值（161616 = 1.863 / -0.43%；000727 = 2.570 / -0.08%），须重算修正。
"""
...
TODAY = "2026-09-23"
CLOSE_D = "2026-09-22"
BASE_TOTAL = 383064.00          # 9/21 收盘链式值（= 9/22 盘前基准，含 9/21 净值补更修正）


def load_snapshot(acct, fn):
    rows = []
    with open(os.path.join(HOLD, acct, fn), encoding="utf-8-sig") as f:
        csv_reader = __import__("csv").reader(f)
...
# ---------- 9/22 盘后档的代理估算值（用于对比修正量） ----------
prev_close = json.load(open(os.path.join(HIST, "portfolio_close_20260922.json"), encoding="utf-8"))
prev_est = prev_close["est_total_pct"]
prev_total = prev_close["total_mv"]
prev_px = {p["code"]: p["est_pct"] for p in prev_close.get("proxy_used", [])}

NAV_TARGET = "2026-09-22"      # 本档目标净值日（A股类）
...
def calc(row):
    ...
    if code6 in ETF_PX:
        return shares * ETF_PX[code6], f"场内 9/22 收盘 {ETF_PX[code6]}", "real"
    if code6 in STOCK_PX:
        return shares * STOCK_PX[code6], f"个股 9/22 收盘 {STOCK_PX[code6]}", "real"
    if code6 in fund_nav:
        nd, nav = fund_nav[code6]
        tag = "real" if nd == NAV_TARGET else ("qdi" if code6 in QDII else "stale")
...
print(f"9/21 收盘链式基准: {BASE_TOTAL:,.2f}")
print(f"9/22 收盘（本档真实净值修正后）总资产: {total_mv:,.2f}")
print(f"  修正后 9/22 当日收益: {corr_pct:+.4f}%  ({total_mv - BASE_TOTAL:+,.2f} 元)")
...
fix = {"date": CLOSE_D, "as_of": "2026-09-22收盘(净值补更修正)",
       ...
       "correction_note": f"盘后档代理估算 {prev_est:+.2f}% → 真实净值修正 {corr_pct:+.4f}%"
                          ...
                          f"161616 估计 {prev_px.get('161616')}% → 真实 -0.43%；000727 估计 {prev_px.get('000727')}% → 真实 -0.08%",
       "fix_source": "fetch_preopen_20260923（F10 直连真实净值）",
...
# ---------- 夏普 / 链路 / 回撤（§3.95/§3.98: 显式优先级 _fix 覆盖，不依赖 glob 顺序） ----------
rf_10y = 1.68
...
print(f"W39(9/21-9/25): {win_cum('2026-09-21', CLOSE_D):+.2f}%  近5日: {sum(rets[-5:]):+.2f}%  近10日: {sum(rets[-10:]):+.2f}%")
print("\n--- 日收益样本序列（★= _fix 覆盖生效） ---")
for dt, p in daily_rets:
...
detail = [{"account": r["account"], "name": r["name"], "code": str(r["code"]), "track": r["track"],
          ...
          for _, r in all_df.sort_values("mv", ascending=False).iterrows()]
out = {"date": TODAY, "as_of": "9/22收盘(修正口径)",
       "snap_total": round(snap_total, 2), "total_mv": round(total_mv, 2),
       "base_total": BASE_TOTAL, "close_20260922_pct": corr_pct,
       ...
       "threshold_all_med": round((0.40 / 0.60 * (total_mv - med) / med - 1) * 100, 2),
       "threshold_a_sh_med": round(((0.40 / 0.60 * (total_mv - med)
                                    - track_amount.get("美股标普医药", 0))
                                   / track_amount.get("A股医药", 1) - 1) * 100, 2),
...
rs = {"date": TODAY, "chain_cum_pct": round(chain, 2), "chain_days": len(daily_rets),
      ...
      "w39_cum_pct": win_cum("2026-09-21", CLOSE_D), "w39_days": 2,
      "sharpe": round(sharpe, 2), "sharpe_n": len(daily_rets),
      "daily_mean": round(mean_d, 4), "daily_sd": round(std_d, 4),
      "max_dd_pct": round(mdd, 2), "last5_cum_pct": round(sum(rets[-5:]), 2),
      "last10_cum_pct": round(sum(rets[-10:]), 2),
      "fix_applied": ["portfolio_close_20260918_fix.json", "portfolio_close_20260921_fix.json",
                      "portfolio_close_20260922_fix.json"],

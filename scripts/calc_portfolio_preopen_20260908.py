# -*- coding: utf-8 -*-
"""盘前档 2026-09-08：组合估值重算（9/7 收盘真实净值全量口径，QDII 广发全球医疗 A/C 9/4 -0.95%/-0.96% 已兑现落地、交银海外互联 9/4 +2.65%）+ 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 9/4、其余 9/7）；场内 ETF/个股用 9/7 收盘价"""
...
print("fund_nav 最新净值（QDII 9/4 兑现、其余 9/7）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 9/4 收盘（etf_intraday.csv 收盘20:00 口径） ----------
ETF_SPOT = {}
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-07" and "收盘" in row.get("note", ""):
...
# ---------- 个股 9/4 收盘 ----------
STOCK_PRICE = {"002410": 8.81, "600438": 11.67}   # 广联达 8.81(-0.56%)、通威 11.67(+2.46%)
...
print(f"\n9/7 收盘口径总资产(净值全量修正): {total_mv:,.2f}")
...
print("\n参考: 9/7 盘后代理口径 384,918.05；本次为场外 9/7 净值全量 + QDII 9/4 兑现修正口径")
...
out = {"date": "2026-09-08", "as_of": "2026-09-07收盘(净值全量修正,QDII 9/4兑现)", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260908.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260908.json")

# -*- coding: utf-8 -*-
"""2026-08-27 盘中: 事件库追加 8 条盘中新闻 (id 017-024) + 历史匹配统计"""
import json, os, glob
from collections import defaultdict

BASE = '/Users/jieyang/Documents/WealthHub'
EVENTS_DIR = os.path.join(BASE, 'data/processed/events')
SENT_FILE = os.path.join(BASE, 'data/processed/news/sentiment-2026-08-27.json')
OUT_FILE = os.path.join(EVENTS_DIR, 'events-2026-08-27.json')
...
TRACK_MAP = {
    ...
    "洋河股份定价（8/27盘中）": "大消费",
    "白酒批价8/27": "大消费",
    "百利天恒定价验证（8/27盘中）": "A股医药",
    "创新药板块8/27盘中分化": "A股医药",
    "理想汽车-W港股企稳（8/27盘中）": "恒生科技",
    "英伟达财报引爆A股/港股AI算力链（8/27盘中）": "恒生科技",
}
def get_track(title):
...
print(f"events-2026-08-27.json 共 {len(combined)} 条 (盘前{len(existing)} + 盘中{len(new_events)})")
for e in new_events:

...
"""2026-09-25 盘中档事件库处理：
  ...
  ④ 事件库 3/5/10 日窗口统计 → history/event_stats_intraday_20260925.json
⚠️ 本档不执行回填：A股/港股 9/24 尚未收盘，9/24 盘前 43 条留空事件参考交易日 = 9/24（盘后档闭环）
...
out = {'date': TODAY, 'window': 'intraday', 'total_events': len(all_ev), 'today_events': len(ev),
       ...
       'note': '本档 A股休市（中秋），9/25 事件参考交易日 = 9/28，一律留空待盘后/9/28 回填；A股医药 3/5/10 日窗口以 399006 创业板指为代理（§3.90）'}
json.dump(out, open(os.path.join(HIST, f'event_stats_intraday_20260925.json'), 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1)
print('\n已保存 event_stats_intraday_20260925.json')

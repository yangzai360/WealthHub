# -*- coding: utf-8 -*-
"""盘前档 2026-09-14(周一) 行情抓取:
1) 场外基金净值增量(9/11 周五净值, 周一早盘全量可抓; QDII T+1~T+2 应更新至 9/11=XLV -0.18%)
2) 美股(美东 9/11 周五收盘, 9/13 已归档 -> 预期全部 SKIP)
3) 港股/A股 spot(返回 9/11 收盘值, 预期 SKIP)
4) 且慢 pmdj 复测
"""
...
# ---------- 1. 场外基金净值增量(9/11 周四净值; QDII 9/10 应出=XLV -0.33% 兑现) ----------
fund_map = {
...
# ---------- 2. 美股: 美东 9/11(周四)收盘, 北京时间 9/11 早间抓取 ----------
...
print('\n--- A股指数 spot 盘前 ---')
try:
    df = retry(ak.stock_zh_index_spot_sina, times=2)
    if df is not None and len(df) > 0:
        import re
        for _, r in df.iterrows():
            code6 = re.sub(r'\D', '', str(r.get('代码', '')))
            nm = str(r.get('名称', ''))
            if code6 in ('000001', '000932', '399006', '000300') or nm in ('上证指数', '中证消费', '创业板指'):
                print(f"  {nm}: {r.get('最新价')} ({r.get('涨跌幅')}%)")
    else:
        print('  A股 spot 盘前空返回')
except Exception as e:
    print(f'  A股 spot FAIL: {e}')

# ---------- 4b. A股指数日线兜底(9/11 收盘, 新浪日线口径) ----------
print('\n--- A股指数日线(9/11 收盘兜底) ---')
for sym, nm in [('sh000001', '上证指数'), ('sh000932', '中证消费'), ('sz399006', '创业板指'), ('sh000300', '沪深300')]:

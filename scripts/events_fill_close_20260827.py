# -*- coding: utf-8 -*-
"""盘后档 2026-08-27：①回填 8/27 当日 24 条（美股标普医药 2 条留空待 8/28）
②追加盘后 8 条事件(id 025-032)并回填 ③全库完整性扫描 ④1日样本统计+方向验证 ⑤3/5日窗口统计更新"""
...
# ---------- 8/27 各赛道实际收盘走势（用于回填） ----------
track_ret = {
    'A股医药': 0.45,         # ETF均值（医疗ETF +0.29% / 医药ETF广发 +0.61%）；创新药 +1.06%、生物医药 +0.95%、医药生物板块 +0.80%（百利天恒 +1.04% 翻红）
    '大消费': 0.04,          # 中证消费 12,489.39 +0.04% 守住12,400（洋河 -2.58% 软着陆确认、白酒概念 -0.15%、主力净流出10.67亿）
    '恒生科技': -0.13,       # HSTECH 4,620.29 -0.13%（恒指 25,565.74 -0.34%、国企指数 -0.51%；南向净买入31.8亿；翰森+16%/百度+5.3%/智谱+12.6%）
    '宏观': 1.13,            # 上证指数 3,956.57 +1.13%（创业板 +1.71%、科创50 +3.77% 领涨、深成指 +1.50%）
    '其他/宽基': -0.46,      # 广联达 0.00% / 通威 -0.92% 平均
    # 美股标普医药留空（8/27 美股未收盘，待 8/28 用 XLV/IYH 回填）
}
def get_ret1d(e):
...
# ---------- 1. 回填 8/27 当日 24 条 ----------
ev26_path = os.path.join(EV, 'events-2026-08-27.json')
ev26 = json.load(open(ev26_path, encoding='utf-8'))
fix26 = 0
for x in ev26:
    ...
    if t in track_ret:
        ...
        fix26 += 1
json.dump(ev26, open(ev26_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
left26 = [x for x in ev26 if get_ret1d(x) is None]
print(f'8/27 当日回填 {fix26} 条；留空 {len(left26)} 条（美股标普医药待 8/28）')
for x in left26:
    print(f'  留空: {x["id"]} [{x["track"]}] {x["title"][:40]}')

# ---------- 2. 追加盘后 8 条事件（id 025-032）+ 回填 ----------
sent = json.load(open(os.path.join(BASE, 'data/processed/news/sentiment-2026-08-27.json'), encoding='utf-8'))
sent_by_title = {s['title']: s for s in sent}
existing_titles = set(e['title'] for e in ev26)

# 历史样本统计（供 reference，不含 8/27）
all_events = []
...
new_events = []
news_all = json.load(open(os.path.join(BASE, 'data/processed/news/news-2026-08-27.json'), encoding='utf-8'))
for n in news_all:
...
combined = ev26 + new_events
json.dump(combined, open(ev26_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n追加盘后事件 {len(new_events)} 条，events-2026-08-27.json 共 {len(combined)} 条')
for e in new_events:
...
# ---------- 5. 3/5 日窗口统计（防御式边界，§3.42） ----------
print('\n--- 3/5 日窗口统计（截至 8/27 收盘数据） ---')
...
def calc_window(ev_dated, idx_series, track_idx, win_days, label):
    ...
    for d, x in ev_dated:
        ...
        # 事件日收盘后连续 win_days 个交易日累计收益；边界防御：range 最大索引 + 1 <= len

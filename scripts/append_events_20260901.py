# -*- coding: utf-8 -*-
"""盘前档 2026-09-01：①事件库追加 18 条（actual_ret_1d 待 9/1 收盘回填）②回填 8/30+8/31 遗留美股标普医药事件（用 XLV 8/31 -0.36%）③全库完整性扫描 ④1日样本+方向验证 ⑤3/5日窗口统计"""
...
NEWS_PATH = os.path.join(BASE, 'data/processed/news/news-2026-09-01.json')
SENT_PATH = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-01.json')
...
for i, s in enumerate(sentiments, 1):
    ...
    evs.append({
        ...
        "reference": {
            ...
            "confidence": None,
            "actual_ret_1d": None,
            "note": "盘前事件,actual_ret_1d 待 9/1(周二)收盘回填",
...
# ---------- 2. 回填 8/30 + 8/31 遗留美股标普医药事件（XLV 8/31 -0.36% / IYH -0.40%，取 XLV） ----------
FILL_IDS = ['N20260830-010', 'N20260830-011', 'N20260830-012',
            'N20260831-010', 'N20260831-011', 'N20260831-012']
FILL_VAL = -0.36
FILL_NOTE = '美股 8/31 收盘回填(XLV -0.36%, 新浪源)'
filled = 0
for f in sorted(glob.glob(os.path.join(EV, 'events-2026-08-3*.json'))):
    path = os.path.join(EV, f)
    data = json.load(open(path, encoding='utf-8'))
    changed = False
    for e in data:
        if e.get('id') in FILL_IDS:
            ref = e.setdefault('reference', {})
            if ref.get('actual_ret_1d') is None:
                ref['actual_ret_1d'] = FILL_VAL
                ref['note'] = FILL_NOTE
                ref['actual_date'] = '2026-08-31'
                changed = True
                filled += 1
                print(f'  回填 {e["id"]} [{e["track"]}] actual_ret_1d={FILL_VAL}')
    if changed:
...
print(f'\n事件库全量累计: {len(all_ev)} 条；actual_ret_1d 留空: {len(nulls)} 条（今日 18 条待回填正常）')
for x in nulls[:10]:
    print(f'  留空: {x.get("id")} [{x.get("track")}] {x.get("title","")[:40]}')
if len(nulls) > 10:
    print(f'  ... 等 {len(nulls)-10} 条')

# -*- coding: utf-8 -*-
"""2026-09-21 盘后档：DeepSeek v4-flash 情绪标注（盘后窗口 14:00-20:00 新增 18 条）
⚠️ §3.88 修复：必须为每条 item 写 `window` 字段（文件级字段不可替代），否则盘后事件入库静默失败
⚠️ §3.96：sentiment-*.json 为 {date, window, count, items:[...]} 结构，不是裸 list
...
SENT = os.path.join(BASE, 'data/processed/news/sentiment-2026-09-21.json')
...
done = {x['title'][:60] for x in items}
todo = [n for n in news if n['title'][:60] not in done]
print(f'待标注 {len(todo)} 条（news {len(news)} / sentiment {len(items)}）')
if not todo:
    raise SystemExit('⚠️ 无待标注条目，终止（防重复标注）')
...
for k, (n, a) in enumerate(results, start=start + 1):
    items.append({'idx': k, 'title': n['title'][:60], 'track': n['track'],
                  'window': WINDOW,
...
sent['items'] = items
sent['count'] = len(items)
sent['window'] = str(sent.get('window', '')) + ' + ' + WINDOW
...
print(f"sentiment 总计 {len(items)} 条")
for x in add:

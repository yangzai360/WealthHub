# -*- coding: utf-8 -*-
"""盘前档 2026-09-03：①事件库追加 19 条（actual_ret_1d 待 9/3 收盘回填）②动态回填 9/2 遗留美股标普医药事件（用 XLV 9/2 +0.75%）③全库完整性扫描 ④1日样本+方向验证 ⑤3/5日窗口统计"""
...
for i, s in enumerate(sentiments, 1):
    ...
    evs.append({
        ...
        "reference": {
            ...
            "confidence": None,
            "actual_ret_1d": None,
            "note": "盘前事件,actual_ret_1d 待 9/3(周四)收盘回填",
...
# ---------- 2. 动态回填 9/2 遗留美股标普医药事件（XLV 9/2 +0.75%） ----------
FILL_VAL = 0.75
FILL_NOTE = '美股 9/2 收盘回填(XLV +0.75%, 新浪源)'
filled = 0
...
print(f'\n事件库全量累计: {len(all_ev)} 条；actual_ret_1d 留空: {len(nulls)} 条（今日 19 条待回填正常）')
for x in nulls[:12]:
    print(f'  留空: {x.get("id")} [{x.get("track")}] {x.get("title","")[:40]}')
if len(nulls) > 12:
    print(f'  ... 等 {len(nulls)-12} 条')

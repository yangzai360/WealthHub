# -*- coding: utf-8 -*-
"""盘前档 2026-09-11：组合估值重算（9/10 收盘真实净值全量口径，QDII 广发全球医疗 A/C 9/9 兑现落地 -0.47%/-0.48%） + 赛道占比 + 集中度检查 + 夏普比率估算
基金净值从 fund_nav.csv 读取最新（QDII 9/9、其余 9/10）；场内 ETF/个股用 9/10 收盘价"""
...
print("fund_nav 最新净值（QDII 9/9 兑现落地、其余 9/10）：")
for code, (d, nav) in sorted(fund_nav.items()):
    print(f"  {code}: {d} {nav}")

# ---------- 场内 ETF 9/10 收盘（etf_intraday.csv 收盘20:00 口径） ----------
ETF_SPOT = {}
with open(os.path.join(HIST, "etf_intraday.csv"), encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        if row["date"] == "2026-09-10" and "收盘" in row.get("note", ""):
            ETF_SPOT[row["code"]] = float(row["price"])
for c in ["513050", "159928", "512170", "513180", "159920", "512880", "515180", "512980", "159938"]:
    print(f"  ETF {c}: {ETF_SPOT.get(c)}")
# ---------- 个股 9/9 收盘 ----------
STOCK_PRICE = {"002410": 8.39, "600438": 11.50}   # 广联达 8.39(-1.87%)、通威 11.50(-1.79%)
...
print(f"\n9/10 收盘口径总资产(净值全量修正,含QDII 9/9兑现): {total_mv:,.2f}")
...
print("\n参考: 9/10 盘后代理口径 377,094.70(基准380,927.32 -1.01%)；本次为 9/10 净值全量+QDII 9/9 兑现落地修正口径")
...
out = {"date": "2026-09-11", "as_of": "2026-09-10收盘(净值全量修正,QDII 9/9兑现落地-0.47%/-0.48%)", "snap_total": round(snap_total, 2),
       ...
       "rf_10y": rf_10y,
       "detail": detail}
with open(os.path.join(HIST, "portfolio_preopen_20260911.json"), "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print("\n已保存 portfolio_preopen_20260911.json")

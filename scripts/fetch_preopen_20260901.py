# -*- coding: utf-8 -*-
"""盘前档 2026-09-01(周二) 行情抓取：场外基金净值增量(8/31周一净值, 周二早盘全量可抓, QDII T+1~T+2至8/28) + 美股8/31(美东周一)收盘新抓"""
...
# ---------- 1. 场外基金净值增量（8/31 周一净值，周二早盘全量可抓，QDII T+1~T+2） ----------
fund_map = {
...
for sym, nm in [('XLV', '美股医疗XLV'), ('IYH', '美股医疗IYH'), ('QQQ', '纳指100ETF'), ('DIA', '道指ETF')]:
    ...
    last_date = dates[-1]
    if (sym == 'XLV' or sym == 'IYH' or sym == 'QQQ' or sym == 'DIA'):
        pass
    # 计算涨跌幅(末两行)
    pct = None
...
# ---------- 3. 港股 spot 盘前尝试（预期失败/返回 8/31 收盘, 沿用即可） ----------
print('\n--- 港股 spot 盘前 ---')
try:
    ...
    if df is not None and len(df) > 0:
        ...
        print('  港股 spot 盘前空返回,沿用 8/31 收盘(恒指 25,566.99/恒科 4,619.87)')
except Exception as e:
    print(f'  港股 spot FAIL: {e}')

# ---------- 4. 且慢 pmdj 复测（连续第 39 日预期空 body） ----------
try:
